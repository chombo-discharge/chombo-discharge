...
#if 0 // Code that I want, but which is non-performant
...
void EBHelmholtzOp::defineCFIVS(){
  for (int dir = 0; dir < SpaceDim; dir++){
    m_loCFIVS[dir].define(m_eblg.getDBL());
    m_hiCFIVS[dir].define(m_eblg.getDBL());

    for (DataIterator dit(m_eblg.getDBL()); dit.ok(); ++dit){
      m_loCFIVS[dir][dit()].define(m_eblg.getDomain(), m_eblg.getDBL()[dit()], m_eblg.getDBL(), dir, Side::Lo);
      m_hiCFIVS[dir][dit()].define(m_eblg.getDomain(), m_eblg.getDBL()[dit()], m_eblg.getDBL(), dir, Side::Hi);
    }
  }
}
...
void EBHelmholtzOp::applyHomogeneousCFBCs(LevelData<EBCellFAB>& a_phi){
  for (DataIterator dit(m_eblg.getDBL()); dit.ok(); ++dit){
	...
	if (cfivsPtr->isPacked() ){
	  const Real m_dxCoar = m_dx*m_refToCoar;
	  const int ihiorlo = sign(sit());
	  FORT_INTERPHOMO(CHF_FRA(a_phi[dit()].getSingleValuedFAB()),
