...
void McPhoto::computeDensity(EBAMRCellData& a_isotropic, const EBAMRCellData& a_phi){
  ...
  MayDay::Error("McPhoto::computeDensity - can't compute density from RTE solution with discrete photons. Maybe you meant to deposit photons?");
}
	...
	if(!checkEB && !checkDom){ // No intersection test necessary, photons is guaranteed to end up on the mesh. 
	  ...
	  if(checkEB) {
	    ...
	    case EBIntersection::Bisection:
	      constactEB = ParticleOps::ebIntersectionBisect(impFunc, oldPos, newPos, pathLen, m_bisectStep, sEB);
	    case EBIntersection::LSF:
