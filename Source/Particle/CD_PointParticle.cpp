...
void
PointParticle::define(const RealVect a_position, const Real a_mass)
{
  this->setMass(a_mass);
...
void
PointParticle::setMass(const Real a_mass)
{
  m_mass = a_mass;
}
...
const Real&
PointParticle::mass() const
{
  return m_mass;
}

int
PointParticle::size() const
{
  return (BinItem::size() + sizeof(m_mass));
}

void
PointParticle::linearOut(void* buf) const
{
...
void
PointParticle::linearIn(void* buf)
{
  Real* buffer = (Real*)buf;
