...
{
  // TLDR: Min(0.0, Max(dx, dy)) is the shortest distance from a point inside the rectangle to one of the edges. It
  // becomes zero if dx > 0 or dy > 0. The
  //       second term max(RealVect::Zero, delta).vectorLength() is funky. In principle we should only take the distance
  //       to be sqrt(dx^2 + dy^2 + dz^2) if we are closest to a "corner", but if we are closest to one of the edges we
  //       have dx*dy < 0.0. We can just set the other component to zero, which is what max(RealVect::Zero, v) does.
  //       Then take the distance anyways.
