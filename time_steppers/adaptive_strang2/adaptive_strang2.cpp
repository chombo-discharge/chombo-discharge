/*!
  @file   adaptive_strang2.cpp
  @brief  Implementation of adaptive_strang2.H
...
typedef adaptive_strang2::cdr_storage     cdr_storage;
typedef adaptive_strang2::poisson_storage poisson_storage;
typedef adaptive_strang2::rte_storage     rte_storage;
typedef adaptive_strang2::sigma_storage   sigma_storage;

adaptive_strang2::adaptive_strang2(){
  m_advective_order     = 2;
  m_minCFL              = 0.1;
  m_maxCFL              = 0.9; // Overriden below
  m_err_thresh          = 1.E-2;
  m_safety              = 0.95;
  m_error_norm          = 2;
  ...
  
  {
	...
	MayDay::Abort("adaptive_strang2::adaptive_strang2 - unknown argument 'splitting'. This must be 'godunov' or 'strang'");
      }
    }
	...
	if(m_verbosity > 2){
	  pout() << "adaptive_strang2::adaptive_strang2 - Turning off advection & source" << endl;
	}
      }
    }
	...
	if(m_verbosity > 2){
	  pout() << "adaptive_strang2::adaptive_strang2 - Turning off diffusion" << endl;
	}
      }
    }
	...
	if(m_verbosity > 2){
	  pout() << "adaptive_strang2::adaptive_strang2 - Turning off rte" << endl;
	}
      }
    }
	...
	if(m_verbosity > 2){
	  pout() << "adaptive_strang2::adaptive_strang2 - Turning off poisson" << endl;
	}
      }
    }
  }
  ...
  if(m_advective_order < 2 || m_advective_order > m_max_advection_order){
    pout() << "adaptive_strang2 ::adaptive_strang2 - order < 1 or order > 3 requested!." << endl;
    MayDay::Abort("adaptive_strang2 ::adaptive_strang2 - order < 1 or order > 3 requested!.");
  }
...
RefCountedPtr<cdr_storage>& adaptive_strang2::get_cdr_storage(const cdr_iterator& a_solverit){
  return m_cdr_scratch[a_solverit.get_solver()];
}

RefCountedPtr<rte_storage>& adaptive_strang2::get_rte_storage(const rte_iterator& a_solverit){
  return m_rte_scratch[a_solverit.get_solver()];
}

Real adaptive_strang2::restrict_dt(){
  return 1.E99;
}

Real adaptive_strang2::get_max_error(){
  CH_TIME("adaptive_strang2::get_max_error");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::get_max_error" << endl;
  }
...
Real adaptive_strang2::advance(const Real a_dt){
  CH_TIME("adaptive_strang2::advance");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::advance" << endl;
  }
  ...
  adaptive_strang2::compute_cdr_sources(m_time + a_dt);
  adaptive_strang2::compute_cdr_velo(m_time + a_dt);
  time_stepper::compute_cdr_diffusion(m_poisson_scratch->get_E_cell(), m_poisson_scratch->get_E_eb());

  // Print diagnostics
  if(m_print_diagno){
    pout() << "\n";
    pout() << "\t adaptive_strang2::advance(Real a_dt) breakdown" << endl;
    pout() << "\t ==============================================\n" << endl;
...
void adaptive_strang2::advance_diffusion(const Real a_time, const Real a_dt){
  CH_TIME("adaptive_strang2::advance_diffusion");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::advance_diffusion" << endl;
  }
...
void adaptive_strang2::advance_tga_diffusion(const Real a_time, const Real a_dt){
  CH_TIME("adaptive_strang2::advance_tga_diffusion");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::advance_tga_diffusion" << endl;
  }
...
void adaptive_strang2::advance_euler_diffusion(const Real a_time, const Real a_dt){
  CH_TIME("adaptive_strang2::advance_euler_diffusion");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::advance_euler_diffusion" << endl;
  }
...
Real adaptive_strang2::advance_fixed(const int a_substeps, const Real a_dt){
  CH_TIME("adaptive_strang2::advance_fixed");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::advance_fixed" << endl;
  }

  MayDay::Abort("adaptive_strang2::advance_fixed - decommissioned for now");
  ...
  for (int step = 0; step < a_substeps; step++){
    ...
    if(m_advective_order == 2){
      ...
      this->store_solvers();
      this->new_advance_rk3(time, a_dt);
    }
    else{
      MayDay::Abort("adaptive_strang2::advance_fixed - unsupported order requested");
    }
...
Real adaptive_strang2::advance_adaptive(int& a_substeps, Real& a_dt, const Real a_time, const Real a_dtc){
  CH_TIME("adaptive_strang2::advance_adaptive");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::advance_adaptive" << endl;
  }

  // Some basic warnings
  if(m_advective_order == 1){
    MayDay::Abort("adaptive_strang2::advance_adaptive - m_advective_order=1 not supported.");
  }
  else if(m_advective_order > m_max_advection_order){
    MayDay::Abort("adaptive_strang2::advance_adaptive - requested advection integration order is too high.");
  }
  ...
  while(sum_dt < a_dtc){
    ...
    if(m_splitting == "godunov"){
      ...
      MayDay::Abort("adaptive_strang2::advance_adaptive - unknown splitting");
    }


    // Advective advance. Solvers have already been filled with necessary stuff (sources, fluxes, etc. )
    if(m_advective_order == 2){
      this->advance_rk2(cur_time, advective_dt, true);
    }
    else if(m_advective_order == 3){
      this->new_advance_rk3(cur_time, advective_dt, true);
    }
    ...
    // Advance diffusion. This completes the embedded formula which always uses Godunov splitting
    this->advance_diffusion(cur_time, diffusive_dt);

    // Do the rest of the time step if we're doing the strang splitting
    if(m_splitting == "strang"){
      this->update_poisson();
      
      if(m_consistent_rte){
	this->update_rte(m_time+a_dt);
      }

      // Must now update source terms and fluxes before actually solving
      this->store_solvers(); // Need to do this again
      this->compute_cdr_gradients();
      ...
      // Do the second solve
      if(m_advective_order == 2){
	this->advance_rk2(cur_time, advective_dt, false);
      }
      else if(m_advective_order == 3){
	this->new_advance_rk3(cur_time, advective_dt, false);
      }
    }
    ...
    const Real new_dt = m_max_error > 0.0 ? (a_dt*sqrt(m_err_thresh/m_max_error)) : 1.E99;
    ...
    const bool accept_order = (a_dt <= m_minCFL*m_dt_cfl) && (m_advective_order >= m_max_advection_order || m_fixed_order); // Can't increase order or decrease step size
    ...
    if(!m_fixed_order){
      if(decr_order && m_advective_order > 2){
	m_advective_order = m_advective_order - 1;
      }
      else if(incr_order && m_advective_order < m_max_advection_order){
	m_advective_order = m_advective_order + 1;
      }
    }
...
void adaptive_strang2::advance_rk2(const Real a_time, const Real a_dt, const bool a_compute_err){
  CH_TIME("adaptive_strang2::advance_rk2");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::advance_rk2" << endl;
  }
  ...
  // u^n resides in cache, put u^1 in the solver. Use scratch for computing L(u^n)
  {
	...
	if(m_splitting == "godunov"){
	  ...
	  data_ops::incr(err, rhs, a_dt); // err = u^n + 2*dt*L(u^n)
	}
      }
    }
  ...
  // u^n resides in temp storage, u^1 resides in solver. Put u^2 in solver at end and use scratch fo computing L(u^1)
  {
    ...
    for (cdr_iterator solver_it = m_cdr->iterator(); solver_it.ok(); ++solver_it){
      ...
      EBAMRCellData& pre = storage->get_previous(); // u^n, solution at start of time step
...
void adaptive_strang2::advance_rk3(const Real a_time, const Real a_dt, const bool a_compute_err){
  CH_TIME("adaptive_strang2::advance_rk3");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::advance_rk3" << endl;
  }
  ...
  // u^n resides in solver. Use this and put u^1 in the solver instead. Use scratch for computing L(u^n)
  {
	...
	if(m_splitting == "godunov"){
	  data_ops::incr(err, rhs, a_dt);     // err = u^n + dt*L(u^n)
	}
	else if (m_splitting == "strang"){
	  data_ops::incr(err, rhs, 2.0*a_dt); // err = u^n + 2*dt*L(u^n)
	}
      }
    }
  ...
  // Before we move further, we advance the error solution
  if(a_compute_err){
    Vector<EBAMRCellData*> states;
    for (cdr_iterator solver_it = m_cdr->iterator(); solver_it.ok(); ++solver_it){
      RefCountedPtr<cdr_storage>& storage = this->get_cdr_storage(solver_it);
      EBAMRCellData& err = storage->get_error();  // Storage for RHS
      states.push_back(&err);
    }

    this->compute_cdr_sources(states, a_time);
    this->compute_cdr_velo(states, a_time);
    this->compute_cdr_domain_states(states);
    this->compute_cdr_domain_fluxes(states, a_time);
    this->compute_cdr_eb_states(states);
    this->compute_cdr_fluxes(states, a_time);

    // Second Runge-Kutta advance
    for (cdr_iterator solver_it = m_cdr->iterator(); solver_it.ok(); ++solver_it){
      RefCountedPtr<cdr_solver>& solver   = solver_it();
      RefCountedPtr<cdr_storage>& storage = this->get_cdr_storage(solver_it);

      EBAMRCellData& err       = storage->get_error();   // Low order solution
      EBAMRCellData& rhs       = storage->get_scratch();  // Storage for RHS
      const EBAMRCellData& src = solver->get_source();    // Source
      const EBAMRCellData& pre = storage->get_previous(); // u^n

      solver->compute_divF(rhs, err, 0.0, true);
      data_ops::scale(rhs, -1.0);                  // RHS = -div(u^1*v^1)
      data_ops::incr(rhs, src, 1.0);               // RHS = S^1 - div(u^1*v^1)

      if(m_splitting == "godunov"){
	data_ops::incr(err, pre, 1.0);
	data_ops::incr(err, rhs, a_dt);
      }
      else if(m_splitting == "strang"){
	data_ops::incr(err, pre, 1.0);
	data_ops::incr(err, rhs, 2.0*a_dt);
      }

      data_ops::scale(err, 0.5);
    }
  }
  ...
  {
    // All these signatures used the solver contents
    this->compute_cdr_gradients();
    this->compute_cdr_domain_states();
    this->compute_cdr_eb_states();
    this->compute_cdr_domain_fluxes(a_time);
...
void adaptive_strang2::new_advance_rk3(const Real a_time, const Real a_dt, const bool a_compute_err){
  CH_TIME("adaptive_strang2::new_advance_rk3");
  if(m_verbosity > 2){
    pout() << "adaptive_strang2::new_advance_rk3" << endl;
  }

  // NOTE: When a_dt comes in, it has been divided by 2 if m_splitting == strang
  ...
  // Before we move further, we advance the error solution. However, this should not be necessary
  // for Godunov splitting 
  if(a_compute_err && m_splitting != "godunov"){
    ...
    for (cdr_iterator solver_it = m_cdr->iterator(); solver_it.ok(); ++solver_it){
      ...
      EBAMRCellData& err       = storage->get_error();    // Low order solution, err = u^n + 2*dt*L(u^n)
      EBAMRCellData& rhs       = storage->get_scratch();  // Storage for RHS
      const EBAMRCellData& src = solver->get_source();    // Source
      const EBAMRCellData& pre = storage->get_previous(); // u^n
      ...
      data_ops::incr(err, pre, 1.0);               // err = 2*u^n + 2*dt*L(u^n)
      data_ops::incr(err, rhs, 2.0*a_dt);          // err = 2^u^n + 2*dt*L(u^n) + 2*dt*L(u^1)
  ...
  // u^n is stored, u^1 resides in solver. Put u^2 in solver at end and use scratch fo computing L(u^1)
  {
    ...
    for (cdr_iterator solver_it = m_cdr->iterator(); solver_it.ok(); ++solver_it){
      ...
      if(a_compute_err && m_splitting == "godunov"){ // For Godunov splitting, the error is more simple to compute
	EBAMRCellData& err = storage->get_error(); // err =   u^1
	data_ops::incr(err, pre, 1.0);             // err =  (u^n + u^1)
	data_ops::incr(err, rhs, a_dt);            // err =  (u^n + u^1 + dt*L(u^1))
	data_ops::scale(err, 0.5);                 // err = -(u^n + u^1 + dt*L(u^1))/2
      }
    }
...
void adaptive_strang2::regrid_internals(){
...
void adaptive_strang2::compute_cdr_gradients(){
...
void adaptive_strang2::compute_cdr_gradients(const Vector<EBAMRCellData*>& a_states){
  CH_TIME("time_stepper::compute_cdr_gradients");
  if(m_verbosity > 5){
    pout() << "time_stepper::compute_cdr_gradients" << endl;
  }
...
void adaptive_strang2::compute_errors(){
...
void adaptive_strang2::compute_dt(Real& a_dt, time_code::which_code& a_timecode){
...
void adaptive_strang2::allocate_cdr_storage(){
  const int ncomp       = 1;
...
void adaptive_strang2::allocate_poisson_storage(){
  const int ncomp = 1;
...
void adaptive_strang2::allocate_rte_storage(){
  const int ncomp       = 1;
...
void adaptive_strang2::allocate_sigma_storage(){
  const int ncomp = 1;
...
void adaptive_strang2::deallocate_internals(){
...
void adaptive_strang2::cache_solutions(){
  CH_TIME("adaptive_strang2::cache_solutions");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::cache_solutions" << endl;
  }
...
void adaptive_strang2::uncache_solutions(){
  CH_TIME("adaptive_strang2::uncache_solutions");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::uncache_solutions" << endl;
  }
...
void adaptive_strang2::compute_E_into_scratch(){
  CH_TIME("adaptive_strang2::compute_E_into_scratch");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_E_into_scratch" << endl;
  }
...
void adaptive_strang2::compute_cdr_velo(const Real a_time){
  CH_TIME("adaptive_strang2::compute_cdr_velo");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_velo" << endl;
  }

  this->compute_cdr_velo(m_cdr->get_states(), a_time);
}

void adaptive_strang2::compute_cdr_velo(const Vector<EBAMRCellData*>& a_states, const Real a_time){
  CH_TIME("adaptive_strang2::compute_cdr_velo(Vector<EBAMRCellData*>, Real)");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_velo(Vector<EBAMRCellData*>, Real)" << endl;
  }
...
void adaptive_strang2::compute_cdr_eb_states(){
  CH_TIME("adaptive_strang2::compute_cdr_eb_states");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_eb_states" << endl;
  }
...
void adaptive_strang2::compute_cdr_eb_states(const Vector<EBAMRCellData*>& a_states){
  CH_TIME("adaptive_strang2::compute_cdr_eb_states");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_eb_states" << endl;
  }
...
void adaptive_strang2::compute_cdr_domain_states(){
  CH_TIME("adaptive_strang2::compute_cdr_domain_states");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_domain_states" << endl;
  }
...
void adaptive_strang2::compute_cdr_domain_states(const Vector<EBAMRCellData*>& a_states){
  CH_TIME("adaptive_strang2::compute_cdr_domain_states");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_domain_states" << endl;
  }
...
void adaptive_strang2::compute_cdr_fluxes(const Real a_time){
  CH_TIME("adaptive_strang2::compute_cdr_fluxes");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_fluxes" << endl;
  }

  this->compute_cdr_fluxes(m_cdr->get_states(), a_time);
}

void adaptive_strang2::compute_cdr_fluxes(const Vector<EBAMRCellData*>& a_states, const Real a_time){
  CH_TIME("adaptive_strang2::compute_cdr_fluxes(Vector<EBAMRCellData*>, Real)");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_fluxes(Vector<EBAMRCellData*>, Real)" << endl;
  }
...
void adaptive_strang2::compute_cdr_domain_fluxes(const Real a_time){
  CH_TIME("adaptive_strang2::compute_cdr_domain_fluxes");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_domain_fluxes" << endl;
  }

  this->compute_cdr_domain_fluxes(m_cdr->get_states(), a_time);
}

void adaptive_strang2::compute_cdr_domain_fluxes(const Vector<EBAMRCellData*>& a_states, const Real a_time){
  CH_TIME("adaptive_strang2::compute_cdr_domain_fluxes(Vector<EBAMRCellData*>, Real)");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_domain_fluxes(Vector<EBAMRCellData*>, Real)" << endl;
  }
...
void adaptive_strang2::compute_sigma_flux(){
  CH_TIME("adaptive_strang2::compute_sigma_flux");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_sigma_flux" << endl;
  }
...
void adaptive_strang2::compute_cdr_sources(const Real a_time){
  CH_TIME("adaptive_strang2::compute_cdr_sources_into_scratch");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_sources_into_scratch" << endl;
  }

  this->compute_cdr_sources(m_cdr->get_states(), a_time);
}

void adaptive_strang2::compute_cdr_sources(const Vector<EBAMRCellData*>& a_states, const Real a_time){
  CH_TIME("adaptive_strang2::compute_cdr_sources(Vector<EBAMRCellData*>, Real)");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_cdr_sources(Vector<EBAMRCellData*>, Real)" << endl;
  }
...
void adaptive_strang2::advance_rte_stationary(const Real a_time){
  CH_TIME("adaptive_strang2::compute_rte_k1_stationary");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::compute_k1_stationary" << endl;
  }
...
void adaptive_strang2::store_solvers(){
  CH_TIME("adaptive_strang2::store_solvers");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::store_solvers" << endl;
  }
...
void adaptive_strang2::restore_solvers(){
  CH_TIME("adaptive_strang2::restore_solvers");
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::restore_solvers" << endl;
  }
...
void adaptive_strang2::update_poisson(){
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::update_poisson" << endl;
  }
...
void adaptive_strang2::update_rte(const Real a_time){
  if(m_verbosity > 5){
    pout() << "adaptive_strang2::update_rte" << endl;
  }
