...
Real rk2_stiff::advance(const Real a_dt){
  ...
  // Source term advancement always comes first, because this is the only one
  ...
  // Advection-diffusion dance
  if(m_do_adv_diff) {   
    this->advance_advection_diffusion(a_dt);  
  }                                           
