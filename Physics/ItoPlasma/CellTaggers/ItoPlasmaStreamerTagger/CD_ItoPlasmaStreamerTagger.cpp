...
ItoPlasmaStreamerTagger::ItoPlasmaStreamerTagger(const RefCountedPtr<ItoPlasmaPhysics>&     a_physics,
						 ...
						 const RefCountedPtr<ComputationalGeometry>& a_computationalGeometry) : ItoPlasmaStreamerTagger() {
  this->define(a_physics, a_timeStepper, a_amr, a_computationalGeometry);
}

void ItoPlasmaStreamerTagger::parseOptions(){
  parseVerbosity();
  parseBoxes();
  parseBuffer();
...
void ItoPlasmaStreamerTagger::parseRuntimeOptions(){
  parseVerbosity();
  parseBoxes();
  parseBuffer();
...
Vector<Real> ItoPlasmaStreamerTagger::tracer(const RealVect         a_pos,
					     const Real             a_time,
					     const Real             a_dx,
					     const RealVect         a_E,
					     const Real             a_min_E,
...
bool ItoPlasmaStreamerTagger::coarsenCell(const RealVect         a_pos,
					  const Real             a_time,
					  const Real             a_dx,
					  const int              a_lvl,
...
bool ItoPlasmaStreamerTagger::refineCell(const RealVect         a_pos,
					 const Real             a_time,
					 const Real             a_dx,
					 const int              a_lvl,
					 const Vector<Real>     a_tracer,
