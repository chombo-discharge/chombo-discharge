...
ItoPlasmaAir3LEA::ItoPlasmaAir3LEA(){
  ...
  m_reactions.emplace("impact_ionization",      ItoPlasmaReaction({m_electronIdx}, {m_electronIdx, m_electronIdx, m_positiveIdx}, {impact_loss}));
...
void ItoPlasmaAir3LEA::readTables(){
...
Real ItoPlasmaAir3LEA::computeDt(const RealVect a_E, const RealVect a_pos, const Vector<Real> a_cdr_densities) const {
  return 1.E99;
}

Real ItoPlasmaAir3LEA::computeAlpha(const RealVect a_E) const {
  const Real E = a_E.vectorLength();

  return m_tables.at("alpha_lfa").getEntry(E);
}

void ItoPlasmaAir3LEA::updateReactionRatesLEA(const RealVect a_E, const Vector<Real> a_mean_energies, const Real a_dx, const Real a_kappa) const {
...
Real ItoPlasmaAir3LEA::excitationRates(const Real a_E) const{
...
Real ItoPlasmaAir3LEA::sergeyFactor(const Real a_O2frac) const{
  return 3E-2 + 0.4*pow(a_O2frac, 0.6);
}

Real ItoPlasmaAir3LEA::photoionizationRate(const Real a_E) const {
  return excitationRates(a_E)*sergeyFactor(m_O2frac)*m_photoi_factor;
}

ItoPlasmaAir3LEA::Electron::Electron(const LookupTable& a_mobility, const LookupTable& a_diffusion) : m_mobility(a_mobility),
...
Real ItoPlasmaAir3LEA::Electron::mobility(const Real a_energy) const {
  const Real energy = Max(a_energy, 30.0);
  return m_mobility.getEntry(energy);
}

Real ItoPlasmaAir3LEA::Electron::diffusion(const Real a_energy) const {
  const Real energy = Max(a_energy, 30.0);
...
Real ItoPlasmaAir3LEA::PhotonZ::getKappa(const RealVect a_pos) const {
