...
void ito_plasma_stepper::setup_ito(){
  ...
  m_ito->setRealm(m_particle_Realm);
}
...
void ito_plasma_stepper::setup_rte(){
  ...
  m_rte->setRealm(m_particle_Realm);
  m_rte->sanityCheck();
}
...
void ito_plasma_stepper::registerRealms(){
  ...
  m_amr->registerRealm(m_particle_Realm);
}
...
void ito_plasma_stepper::compute_ito_mobilities_lfa(Vector<EBAMRCellData*>& a_meshMobilities, const EBAMRCellData& a_E, const Real a_time){
  ...
  for (auto solver_it = m_ito->iterator(); solver_it.ok(); ++solver_it){
    ...
    if(solver->isMobile()){
      ...
      m_amr->averageDown(*a_meshMobilities[idx], m_particle_Realm, m_phase);
      m_amr->interpGhost(*a_meshMobilities[idx], m_particle_Realm, m_phase);
#endif
...
void ito_plasma_stepper::compute_ito_diffusion_lfa(Vector<EBAMRCellData*>&       a_diffusionCoefficient_funcs,
						   const Vector<EBAMRCellData*>& a_densities,
  ...
  for (auto solver_it = m_ito->iterator(); solver_it.ok(); ++solver_it){
    ...
    if(solver->isDiffusive()){
      ...
      m_amr->averageDown(*a_diffusionCoefficient_funcs[idx], m_particle_Realm, m_phase);
      m_amr->interpGhost(*a_diffusionCoefficient_funcs[idx], m_particle_Realm, m_phase);
#endif
...
void ito_plasma_stepper::compute_ito_diffusion_lfa(Vector<LevelData<EBCellFAB>* >&       a_diffusionCoefficient,
...
void ito_plasma_stepper::compute_ito_diffusion_lfa(Vector<EBCellFAB*>&       a_diffusionCoefficient,
...
void ito_plasma_stepper::compute_reactive_particles_per_cell(LevelData<EBCellFAB>& a_ppc, const int a_level){
  ...
  const DisjointBoxLayout& dbl = m_amr->getGrids(m_particle_Realm)[a_level];
  const EBISLayout& ebisl      = m_amr->getEBISLayout(m_particle_Realm, m_phase)[a_level];
...
void ito_plasma_stepper::compute_reactive_particles_per_cell(EBCellFAB& a_ppc, const int a_level, const DataIndex a_dit, const Box a_box, const EBISBox& a_ebisbox){
  ...
  for (auto solver_it = m_ito->iterator(); solver_it.ok(); ++solver_it){
    ...
    // Irregular cells.
    VoFIterator& vofit = (*m_amr->getVofIterator(m_particle_Realm, m_phase)[a_level])[a_dit];
    for (vofit.reset(); vofit.ok(); ++vofit){
      const VolIndex& vof       = vofit();
...
void ito_plasma_stepper::reconcile_particles(const EBCellFAB& a_newParticlesPerCell,
  ...
  const EBISBox& ebisbox = m_amr->getEBISLayout(m_particle_Realm, m_phase)[a_level][a_dit];
  const EBISBox& ebgraph = m_amr->getEBISLayout(m_particle_Realm, m_phase)[a_level][a_dit];
  ...
  for (auto solver_it = m_rte->iterator(); solver_it.ok(); ++solver_it){
    ...
    particle_container<photon>& solverSourPhotons = solver->getSource_photons();
...
void ito_plasma_stepper::advance_reaction_network(Vector<LayoutData<BinFab<ito_particle> >* >& a_particles,
  ...
  const DisjointBoxLayout& dbl = m_amr->getGrids(m_particle_Realm)[a_lvl];
...
void ito_plasma_stepper::advance_reaction_network(Vector<BinFab<ito_particle>* >& a_particles,
  ...
  const EBISBox& ebisbox = m_amr->getEBISLayout(m_particle_Realm, m_phase)[a_lvl][a_dit];
  const EBISBox& ebgraph = m_amr->getEBISLayout(m_particle_Realm, m_phase)[a_lvl][a_dit];
  ...
  VoFIterator& vofit = (*m_amr->getVofIterator(m_particle_Realm, m_phase)[a_lvl])[a_dit];
  for (vofit.reset(); vofit.ok(); ++vofit){
    const VolIndex vof = vofit();
    const IntVect iv   = vof.gridIndex();
    const RealVect pos = prob_lo + a_dx*(RealVect(iv) + 0.5*RealVect::Unit);
...
void ito_plasma_stepper::advance_photons(const Real a_dt){
  ...
  for (auto solver_it = m_rte->iterator(); solver_it.ok(); ++solver_it){
    ...
    particle_container<photon>& sourcePhotons  = solver->getSource_photons();
...
void ito_plasma_stepper::compute_EdotJ_source(){
  ...
  for (auto solver_it = m_ito->iterator(); solver_it.ok(); ++solver_it){
    ...
    if(q != 0 && solver->isDiffusive()){
      ...
      m_amr->computeGradient(m_particle_scratchD, m_particle_scratch1, m_particle_Realm, m_phase);
