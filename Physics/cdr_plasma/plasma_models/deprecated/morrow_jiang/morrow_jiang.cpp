/*!
  @file   morrow_jiang.cpp
  @brief  Implementation of morrow_jiang.H
  @author Robert Marskar
  @date   Jan. 2018
*/

#include "morrow_jiang.H"
#include "units.H"
#include "data_ops.H"
...
void morrow_jiang::network_tau(Vector<Real>&          a_particle_sources,
  ...
  int sp = 0; // Number of positive ions produced
  int sm = 0; // Number of negative ions produced
  int si = 0; // Number of excited states produced
  ...
  const Real xi = x[m_nexci_idx];
  
  const int Xe = X[m_nelec_idx]; // Integer # of electrons
  const int Xp = X[m_nplus_idx]; // Integer # of positive ions
  const int Xm = X[m_nminu_idx]; // Integer # of negative ions
  const int Xi = X[m_nexci_idx]; // Integer # of excited states
  ...
  // Reaction 5: e + M => M*
  const Real A5 = A1*m_exc_eff;
  const int  S5 = poisson_reaction(A5, a_dt);
  si += S5;

  // Reaction 6: M* => M
  const Real A6 = Xi/m_quench_tau;
  const int  S6 = poisson_reaction(A6, a_dt);
  si -= S6;

  // Reaction 7: M* => M + y
  const Real A7 = Xi/m_exc_tau;
  ...
  Real& Sp = a_particle_sources[m_nplus_idx];
  Real& Sm = a_particle_sources[m_nminu_idx];
  Real& Si = a_particle_sources[m_nexci_idx];
...
Vector<RealVect> morrow_jiang::compute_cdr_velocities(const Real         a_time,
						      const RealVect     a_pos,
						      const RealVect     a_E,
...
RealVect morrow_jiang::compute_ve(const RealVect a_E) const{
...
RealVect morrow_jiang::compute_vp(const RealVect a_E) const{
...
RealVect morrow_jiang::compute_vn(const RealVect a_E) const{
...
Real morrow_jiang::compute_alpha(const RealVect a_E) const{
  Real alpha    = 0.;
  Real alphabyN = 0.;
...
Real morrow_jiang::compute_eta(const RealVect a_E) const{
...
Real morrow_jiang::compute_eta2(const RealVect a_E) const{
  Real eta2    = 0.;
  Real eta2byN = 0.;
...
Real morrow_jiang::compute_eta3(const RealVect a_E) const{
  const RealVect E = a_E*1.E-2;         // Morrow-Lowke wants E in V/cm
  const Real Emag  = E.vectorLength();  //
  const Real N     = m_N*1.E-6;         // Morrow-Lowke weants N in cm^3
  const Real EbyN  = Emag/N;            // This is now V/cm^2
...
Real morrow_jiang::compute_beta(const RealVect a_E) const{
  Real beta = 2.0E-7;
  beta *= 1.E-6; // Morrow-Lowke expression is in cm^3. Make it m^3
  return beta;
}

Real morrow_jiang::compute_De(const RealVect a_E) const{
  const RealVect E  = a_E*1.E-2;                 // Morrow-Lowke wants E in V/cm
  const Real Emag   = E.vectorLength();          //
  const Real N      = m_N*1.E-6;                 // Morrow-Lowke weants N in cm^3
  const Real EbyN   = Emag/N;                    //
...
Vector<Real> morrow_jiang::compute_cdr_diffusion_coefficients(const Real         a_time,
							      const RealVect     a_pos,
							      const RealVect     a_E,
							      const Vector<Real> a_cdr_densities) const{
...
Vector<Real> morrow_jiang::compute_cdr_fluxes(const Real         a_time,
					      const RealVect     a_pos,
...
Vector<Real> morrow_jiang::compute_cdr_domain_fluxes(const Real           a_time,
						     const RealVect       a_pos,
						     const int            a_dir,
...
Vector<Real> morrow_jiang::compute_cdr_electrode_fluxes(const Real         a_time,
							const RealVect     a_pos,
							const RealVect     a_normal,
							const RealVect     a_E,
							const Vector<Real> a_cdr_densities,
...
Vector<Real> morrow_jiang::compute_cdr_dielectric_fluxes(const Real         a_time,
							 const RealVect     a_pos,
							 const RealVect     a_normal,
							 const RealVect     a_E,
							 const Vector<Real> a_cdr_densities,
...
int morrow_jiang::poisson_reaction(const Real a_propensity, const Real a_dt) const{
  int value = 0;
...
int morrow_jiang::binomial_trials(const int a_trials, const Real a_p) const {
  std::binomial_distribution<int> dist(a_trials, a_p);
  return dist(*m_rng);
}

Real morrow_jiang::initial_sigma(const Real a_time, const RealVect a_pos) const{
  return 0.;
}
...
void morrow_jiang::instantiate_species(){
  ...
  // Instantiate species
  m_species[m_nelec_idx]  = RefCountedPtr<species>      (new morrow_jiang::electron());
...
void morrow_jiang::parse_gas_params(){
  ParmParse pp("morrow_jiang");
  std::string str;
  ...
  pp.get("excited_lifetime",           m_exc_tau);
  ...
  // Set the quenching lifetime
  m_quench_tau = m_exc_tau*m_pq/m_p;
}

void morrow_jiang::parse_see(){
...
void morrow_jiang::parse_reaction_settings(){
  ParmParse pp("morrow_jiang");
  std::string str;
  ...
  if(str == "ssa"){
    ...
    MayDay::Abort("morrow_jiang::parse_reaction_settings - stop!");
  }
...
void morrow_jiang::parse_initial_particles(){

  List<Particle> ion_p;
  List<Particle> exc_p;

  // Get types of particles
  add_uniform_pairs(ion_p);
  add_gaussian_pairs(ion_p);

  add_uniform_excited(exc_p);
...
void morrow_jiang::add_uniform_pairs(List<Particle>& a_particles){
  ...
  ParmParse pp("morrow_jiang");
  pp.get("uniform_pairs",  num_particles);
...
void morrow_jiang::add_uniform_excited(List<Particle>& a_particles){
  ...
  // Create uniform particles
  ParmParse pp("morrow_jiang");
  pp.get("uniform_excited",  num_particles);
  pp.get("particle_weight", weight);
  num_uniform_particles = round(num_particles);
...
void morrow_jiang::add_gaussian_pairs(List<Particle>& a_particles){
  ...
  pp.get("particle_weight", weight);
  pp.get("gaussian_pairs", num_particles);
  pp.get("gaussian_pairs_radius",    gaussian_radius);
  pp.getarr("gaussian_pairs_center", vec, 0, SpaceDim); gaussian_center = RealVect(D_DECL(vec[0], vec[1], vec[2]));
...
void morrow_jiang::add_gaussian_excited(List<Particle>& a_particles){
  ...
  // Create Gaussian seed particles
  pp.get("particle_weight", weight);
  pp.get("gaussian_excited", num_particles);
  pp.get("gaussian_excited_radius",    gaussian_radius);
  pp.getarr("gaussian_excited_center", vec, 0, SpaceDim); gaussian_center = RealVect(D_DECL(vec[0], vec[1], vec[2]));
