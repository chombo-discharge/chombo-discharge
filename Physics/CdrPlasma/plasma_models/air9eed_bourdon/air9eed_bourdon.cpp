...
void air9eed_bourdon::read_initEed(){
  read_file_entries(m_initEed, air9eed_bourdon::s_bolsig_energy_E);
...
Real air9eed_bourdon::computeAlpha(const RealVect a_E) const{
...
Vector<Real> air9eed_bourdon::computeCdrDiffusionCoefficients(const Real         a_time,
...
Vector<RealVect> air9eed_bourdon::computeCdrDriftVelocities(const Real         a_time,
							 const RealVect     a_pos,
							 const RealVect     a_E,
							 const Vector<Real> a_cdr_densities) const {
...
void air9eed_bourdon::advanceReactionNetwork(Vector<Real>&          a_cdr_sources,
...
Vector<Real> air9eed_bourdon::computeCdrDomainFluxes(const Real           a_time,
							const RealVect       a_pos,
							const int            a_dir,
...
Vector<Real> air9eed_bourdon::computeCdrDielectricFluxes(const Real         a_time,
							    const RealVect     a_pos,
...
Real air9eed_bourdon::initialSigma(const Real a_time, const RealVect a_pos) const {return 0.0;}
...
Real air9eed_bourdon::initEed(const RealVect a_pos, const Real a_time, const RealVect a_E){
  const Real EbyN = (a_E/(m_N*Units::Td)).vectorLength();
...
Vector<Real> air9eed_bourdon::computeCdrElectrodeFluxes(const Real         a_time,
							   const RealVect     a_pos,
							   const RealVect     a_normal,
							   const RealVect     a_E,
							   const Vector<Real> a_cdr_densities,
