...
Vector<Real> air_eed_tagger::tracer(const RealVect         a_pos,
  ...
  tracers[1] = plaskin->computeAlpha_eff(a_E.vectorLength());


  return tracers;
}
bool air_eed_tagger::coarsenCell(const RealVect         a_pos,
				  const Real             a_time,
				  const Real             a_dx,
				  const int              a_lvl,
...
bool air_eed_tagger::refineCell(const RealVect         a_pos,
				 const Real             a_time,
				 const Real             a_dx,
				 const int              a_lvl,
				 const Vector<Real>     a_tracer,
