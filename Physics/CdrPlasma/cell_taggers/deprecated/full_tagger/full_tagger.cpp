...
void full_tagger::allocate_storage(){
  ...
  m_cdr_densities.resize(m_physics->getNumCdrSpecies());
  m_cdr_gradients.resize(m_physics->getNumCdrSpecies());
  m_rte_densities.resize(m_physics->getNumRtSpecies());
...
void full_tagger::computeTracers(){
  ...
  Vector<Real>     rte_phi;
  cdr_phi.resize(m_physics->getNumCdrSpecies());
  cdr_gra.resize(m_physics->getNumCdrSpecies());
  rte_phi.resize(m_physics->getNumRtSpecies());
