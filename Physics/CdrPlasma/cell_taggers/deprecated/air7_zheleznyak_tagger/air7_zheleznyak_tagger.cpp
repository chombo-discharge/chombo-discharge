/*!
  @file   air7_zheleznyak_tagger.cpp
  @brief  Implementation air7_zheleznyak_tagger.H
  @author Robert Marskar
  @date   Feb. 2018
*/

#include "air7_zheleznyak_tagger.H"
#include "air7_zheleznyak.H"

#include <ParmParse.H>

air7_zheleznyak_tagger::air7_zheleznyak_tagger(){
  m_name = "air7_zheleznyak_tagger";
  m_num_tracers = 2;
}
...
void air7_zheleznyak_tagger::parseOptions(){
  parseVerbosity();
  parseBoxes();
  parseBuffer();
...
Vector<Real> air7_zheleznyak_tagger::tracer(const RealVect         a_pos,
					    const Real             a_time,
					    const Real             a_dx,
					    const RealVect         a_E,
					    const Real             a_min_E,
  ...
  const air7_zheleznyak* plaskin = static_cast<air7_zheleznyak*> (&(*m_plaskin));
  
  Vector<Real> tracers(m_num_tracers);
  tracers[0] = a_E.vectorLength()/a_max_E;
  tracers[1] = plaskin->computeAlpha_eff(a_E);


  return tracers;
}
bool air7_zheleznyak_tagger::coarsen_cell(const RealVect         a_pos,
					  const Real             a_time,
					  const Real             a_dx,
					  const int              a_lvl,
...
bool air7_zheleznyak_tagger::refine_cell(const RealVect         a_pos,
					 const Real             a_time,
					 const Real             a_dx,
					 const int              a_lvl,
					 const Vector<Real>     a_tracer,
