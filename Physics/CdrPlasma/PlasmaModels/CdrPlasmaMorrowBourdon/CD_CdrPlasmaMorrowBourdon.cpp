...
void
CdrPlasmaMorrowBourdon::parseGas()
{
...
void
CdrPlasmaMorrowBourdon::parsePhotoi()
{
  ParmParse pp("CdrPlasmaMorrowBourdon");
...
void
CdrPlasmaMorrowBourdon::parseSEE()
{
  ParmParse pp("CdrPlasmaMorrowBourdon");
...
void
CdrPlasmaMorrowBourdon::parseBc()
{
...
void
CdrPlasmaMorrowBourdon::initSpecies()
{
  m_numCdrSpecies = 3;
  m_numRtSpecies  = 3;
...
void
CdrPlasmaMorrowBourdon::advanceReactionNetwork(Vector<Real>&          a_particle_sources,
                                               ...
                                               const Real             a_kappa) const
{
  ...
  const CdrPlasmaMorrowBourdon::PhotonOne* Photon1 =
    static_cast<CdrPlasmaMorrowBourdon::PhotonOne*>(&(*m_rtSpecies[m_Photon1_idx]));
  const CdrPlasmaMorrowBourdon::PhotonTwo* Photon2 =
    static_cast<CdrPlasmaMorrowBourdon::PhotonTwo*>(&(*m_rtSpecies[m_Photon2_idx]));
  const CdrPlasmaMorrowBourdon::PhotonThree* Photon3 =
    static_cast<CdrPlasmaMorrowBourdon::PhotonThree*>(&(*m_rtSpecies[m_Photon3_idx]));
  ...
  const Real Sph =
    m_photo_eff * Units::c * m_fracO2 * m_p *
    (Photon1->getA() * a_Photon_densities[m_Photon1_idx] + Photon2->getA() * a_Photon_densities[m_Photon2_idx] +
...
Vector<RealVect>
CdrPlasmaMorrowBourdon::computeCdrDriftVelocities(const Real         a_time,
                                                  const RealVect     a_pos,
                                                  const RealVect     a_E,
...
RealVect
CdrPlasmaMorrowBourdon::computeVe(const RealVect a_E) const
{
...
RealVect
CdrPlasmaMorrowBourdon::computeVp(const RealVect a_E) const
{
...
RealVect
CdrPlasmaMorrowBourdon::computeVn(const RealVect a_E) const
{
...
Real
CdrPlasmaMorrowBourdon::computeAlpha(const Real a_E, const RealVect a_pos) const
{
  Real alpha    = 0.;
  Real alphabyN = 0.;
...
Real
CdrPlasmaMorrowBourdon::computeEta(const RealVect a_E) const
{
...
Real
CdrPlasmaMorrowBourdon::computeEta2(const RealVect a_E) const
{
  Real eta2    = 0.;
  Real eta2byN = 0.;
...
Real
CdrPlasmaMorrowBourdon::computeEta3(const RealVect a_E) const
{
  const RealVect E    = a_E * 1.E-2;      // Morrow-Lowke wants E in V/cm
  const Real     Emag = E.vectorLength(); //
  const Real     N    = m_N * 1.E-6;      // Morrow-Lowke weants N in cm^3
...
Real
CdrPlasmaMorrowBourdon::computeBeta(const RealVect a_E) const
{
  Real beta = 2.0E-7;
...
Real
CdrPlasmaMorrowBourdon::computeDe(const RealVect a_E) const
{
  const RealVect E    = a_E * 1.E-2;      // Morrow-Lowke wants E in V/cm
  const Real     Emag = E.vectorLength(); //
  const Real     N    = m_N * 1.E-6;      // Morrow-Lowke weants N in cm^3
...
Vector<Real>
CdrPlasmaMorrowBourdon::computeCdrDiffusionCoefficients(const Real         a_time,
                                                        const RealVect     a_pos,
                                                        const RealVect     a_E,
                                                        const Vector<Real> a_cdr_densities) const
{
...
Vector<Real>
CdrPlasmaMorrowBourdon::computeCdrDielectricFluxes(const Real         a_time,
                                                   const RealVect     a_pos,
...
Vector<Real>
CdrPlasmaMorrowBourdon::computeCdrElectrodeFluxes(const Real         a_time,
                                                  const RealVect     a_pos,
                                                  const RealVect     a_normal,
                                                  const RealVect     a_E,
                                                  const Vector<Real> a_cdr_densities,
...
Vector<Real>
CdrPlasmaMorrowBourdon::computeCathodeFlux(const Vector<Real> a_extrapolated_fluxes,
...
Vector<Real>
CdrPlasmaMorrowBourdon::computeAnodeFlux(const Vector<Real> a_extrapolated_fluxes,
                                         const Vector<Real> a_ion_densities,
                                         const Vector<Real> a_ion_velocities,
                                         const Vector<Real> a_Photon_fluxes,
                                         const RealVect     a_E,
...
Vector<Real>
CdrPlasmaMorrowBourdon::computeCdrDomainFluxes(const Real           a_time,
                                               const RealVect       a_pos,
                                               const int            a_dir,
...
Real
CdrPlasmaMorrowBourdon::initialSigma(const Real a_time, const RealVect a_pos) const
{
  return 0.;
}
...
Real
CdrPlasmaMorrowBourdon::Electron::initialData(const RealVect a_pos, const Real a_time) const
{
...
Real
CdrPlasmaMorrowBourdon::PositiveSpecies::initialData(const RealVect a_pos, const Real a_time) const
{
  const Real factor = (a_pos - m_seed_pos).vectorLength() / m_seed_radius;
  const Real seed   = m_seed_density * exp(-factor * factor);
...
Real
CdrPlasmaMorrowBourdon::negative_species::initialData(const RealVect a_pos, const Real a_time) const
{
  return 0.;
}
...
Real
CdrPlasmaMorrowBourdon::PhotonOne::getAbsorptionCoefficient(const RealVect a_pos) const
{
...
Real
CdrPlasmaMorrowBourdon::PhotonTwo::getAbsorptionCoefficient(const RealVect a_pos) const
{
  return m_lambda * m_pO2 / sqrt(3.0); // I think this is correct.
}
...
Real
CdrPlasmaMorrowBourdon::PhotonThree::getAbsorptionCoefficient(const RealVect a_pos) const
{
  return m_lambda * m_pO2 / sqrt(3.0); // I think this is correct.
}
