...
Real
CdrPlasmaAir7Zheleznyak::Electron::initialData(const RealVect a_pos, const Real a_time) const
{
...
Real
CdrPlasmaAir7Zheleznyak::N2plus::initialData(const RealVect a_pos, const Real a_time) const
{
  const Real factor = (a_pos - m_seed_pos).vectorLength();
...
Real
CdrPlasmaAir7Zheleznyak::O2plus::initialData(const RealVect a_pos, const Real a_time) const
{
  const Real factor = (a_pos - m_seed_pos).vectorLength();
  return m_frac * (m_uniform_density + m_seed_density * exp(-factor * factor / (m_seed_rad * m_seed_rad)));
}
...
Real
CdrPlasmaAir7Zheleznyak::uv_Photon::getAbsorptionCoefficient(const RealVect a_pos) const
{
  return getRandomKappa();
}

Real
CdrPlasmaAir7Zheleznyak::uv_Photon::getRandomKappa() const
{
