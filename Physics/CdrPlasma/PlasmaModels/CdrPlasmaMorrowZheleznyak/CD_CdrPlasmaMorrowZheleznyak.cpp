...
void
CdrPlasmaMorrowZheleznyak::advanceReactionNetwork(Vector<Real>&          a_particle_sources,
                                                  ...
                                                  const Real             a_kappa) const
{
  for (int i = 0; i < a_particle_densities.size(); i++) {
    if (a_particle_densities[i] < 0.0)
      MayDay::Abort("CdrPlasmaMorrowZheleznyak::advance_network - shouldn't happen");
  }
...
Real
CdrPlasmaMorrowZheleznyak::excitationRates(const Real a_E) const
{
...
Real
CdrPlasmaMorrowZheleznyak::sergeyFactor(const Real a_O2frac) const
{
...
void
CdrPlasmaMorrowZheleznyak::networkRRE(Vector<Real>&          a_particle_sources,
                                      Vector<Real>&          a_Photon_sources,
                                      const Vector<Real>     a_particle_densities,
                                      const Vector<RealVect> a_particle_gradients,
                                      const Vector<Real>     a_Photon_densities,
...
void
CdrPlasmaMorrowZheleznyak::networkTau(Vector<Real>&          a_particle_sources,
                                      Vector<Real>&          a_Photon_sources,
                                      const Vector<Real>     a_particle_densities,
                                      const Vector<RealVect> a_particle_gradients,
                                      const Vector<Real>     a_Photon_densities,
...
// Tau leaping method
void
CdrPlasmaMorrowZheleznyak::networkSSA(Vector<Real>&          a_particle_sources,
                                      Vector<Real>&          a_Photon_sources,
                                      const Vector<Real>     a_particle_densities,
                                      const Vector<RealVect> a_particle_gradients,
                                      const Vector<Real>     a_Photon_densities,
...
Vector<RealVect>
CdrPlasmaMorrowZheleznyak::computeCdrDriftVelocities(const Real         a_time,
                                                     const RealVect     a_pos,
                                                     const RealVect     a_E,
...
RealVect
CdrPlasmaMorrowZheleznyak::computeVe(const RealVect a_E) const
{
...
RealVect
CdrPlasmaMorrowZheleznyak::computeVp(const RealVect a_E) const
{
...
RealVect
CdrPlasmaMorrowZheleznyak::computeVn(const RealVect a_E) const
{
...
Real
CdrPlasmaMorrowZheleznyak::computeAlpha(const Real a_E, const RealVect a_pos) const
{
  Real alpha    = 0.;
  Real alphabyN = 0.;
...
Real
CdrPlasmaMorrowZheleznyak::computeEta(const RealVect a_E) const
{
...
Real
CdrPlasmaMorrowZheleznyak::computeEta2(const RealVect a_E) const
{
  Real eta2    = 0.;
  Real eta2byN = 0.;
...
Real
CdrPlasmaMorrowZheleznyak::computeEta3(const RealVect a_E) const
{
  const RealVect E    = a_E * 1.E-2;      // Morrow-Lowke wants E in V/cm
  const Real     Emag = E.vectorLength(); //
  const Real     N    = m_N * 1.E-6;      // Morrow-Lowke weants N in cm^3
...
Real
CdrPlasmaMorrowZheleznyak::computeBeta(const RealVect a_E) const
{
  Real beta = 2.0E-7;
...
Real
CdrPlasmaMorrowZheleznyak::computeDe(const RealVect a_E) const
{
  const RealVect E    = a_E * 1.E-2;      // Morrow-Lowke wants E in V/cm
  const Real     Emag = E.vectorLength(); //
  const Real     N    = m_N * 1.E-6;      // Morrow-Lowke weants N in cm^3
...
Vector<Real>
CdrPlasmaMorrowZheleznyak::computeCdrDiffusionCoefficients(const Real         a_time,
                                                           const RealVect     a_pos,
                                                           const RealVect     a_E,
                                                           const Vector<Real> a_cdr_densities) const
{
...
Vector<Real>
CdrPlasmaMorrowZheleznyak::computeCdrFluxes(const Real         a_time,
                                            const RealVect     a_pos,
...
Vector<Real>
CdrPlasmaMorrowZheleznyak::computeCdrDomainFluxes(const Real           a_time,
                                                  const RealVect       a_pos,
                                                  const int            a_dir,
...
Vector<Real>
CdrPlasmaMorrowZheleznyak::computeCdrElectrodeFluxes(const Real         a_time,
                                                     const RealVect     a_pos,
                                                     const RealVect     a_normal,
                                                     const RealVect     a_E,
                                                     const Vector<Real> a_cdr_densities,
...
Vector<Real>
CdrPlasmaMorrowZheleznyak::computeCdrDielectricFluxes(const Real         a_time,
                                                      const RealVect     a_pos,
                                                      const RealVect     a_normal,
                                                      const RealVect     a_E,
                                                      const Vector<Real> a_cdr_densities,
...
int
CdrPlasmaMorrowZheleznyak::poissonReaction(const Real a_propensity, const Real a_dt) const
{
  int        value = 0;
...
Real
CdrPlasmaMorrowZheleznyak::initialSigma(const Real a_time, const RealVect a_pos) const
{
  return 0.;
}
...
Real
CdrPlasmaMorrowZheleznyak::Electron::initialData(const RealVect a_pos, const Real a_time) const
{
...
Real
CdrPlasmaMorrowZheleznyak::PositiveSpecies::initialData(const RealVect a_pos, const Real a_time) const
{
  const Real factor = (a_pos - m_seed_pos).vectorLength() / m_seed_radius;
  const Real seed   = m_seed_density * exp(-factor * factor);
...
Real
CdrPlasmaMorrowZheleznyak::negative_species::initialData(const RealVect a_pos, const Real a_time) const
{
  return 0.;
}
...
Real
CdrPlasmaMorrowZheleznyak::UvPhoton::getAbsorptionCoefficient(const RealVect a_pos) const
{
  return this->getRandomKappa();
}

Real
CdrPlasmaMorrowZheleznyak::UvPhoton::getRandomKappa() const
{
...
void
CdrPlasmaMorrowZheleznyak::parseGasParameters()
{
  ParmParse   pp("CdrPlasmaMorrowZheleznyak");
  std::string str;
...
void
CdrPlasmaMorrowZheleznyak::parseAlphaCorr()
{
  ParmParse pp("CdrPlasmaMorrowZheleznyak");
...
void
CdrPlasmaMorrowZheleznyak::parseSEE()
{
...
void
CdrPlasmaMorrowZheleznyak::parseDomainBc()
{
...
void
CdrPlasmaMorrowZheleznyak::parseEbBC()
{
  ...
  pp.get("extrap_electrode", str);
  m_extrap_electrode_ebbc = (str == "true") ? true : false;
  pp.get("extrap_dielectric", str);
  m_extrap_dielectric_ebbc = (str == "true") ? true : false;
}

void
CdrPlasmaMorrowZheleznyak::parseReactionSettings()
{
  ParmParse   pp("CdrPlasmaMorrowZheleznyak");
  std::string str;
...
void
CdrPlasmaMorrowZheleznyak::parseInitialParticles()
{
...
void
CdrPlasmaMorrowZheleznyak::addUniformParticles(List<Particle>& a_particles)
{
  ...
  Vector<Real> vec(SpaceDim);
  {
    ...
    pp1.getarr("lo_corner", vec, 0, SpaceDim);
    lo = RealVect(D_DECL(vec[0], vec[1], vec[2]));
    pp1.getarr("hi_corner", vec, 0, SpaceDim);
    hi = RealVect(D_DECL(vec[0], vec[1], vec[2]));
  }
...
void
CdrPlasmaMorrowZheleznyak::addGaussianParticles(List<Particle>& a_particles)
{
  ...
  pp.getarr("gaussian_center", vec, 0, SpaceDim);
  gaussian_center = RealVect(D_DECL(vec[0], vec[1], vec[2]));
...
RealVect
CdrPlasmaMorrowZheleznyak::randomGaussian(const Real a_rad)
{
...
RealVect
CdrPlasmaMorrowZheleznyak::randomDirection()
{
#if CH_SPACEDIM == 2
...
#if CH_SPACEDIM == 2
RealVect
CdrPlasmaMorrowZheleznyak::randomDirection2D()
{
...
#if CH_SPACEDIM == 3
RealVect
CdrPlasmaMorrowZheleznyak::randomDirection3D()
{
  const Real EPS = 1.E-8;
  Real       x1  = 2.0;
  Real       x2  = 2.0;
