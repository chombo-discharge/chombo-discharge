...
void
CdrPlasmaAir3Bourdon::initSpecies()
{
  m_numCdrSpecies = 3;
  m_numRtSpecies  = 3;
...
void
CdrPlasmaAir3Bourdon::parseDomainBc()
{
...
void
CdrPlasmaAir3Bourdon::advanceReactionNetwork(Vector<Real>&          a_particle_sources,
                                             ...
                                             const Real             a_kappa) const
{
  ...
  const CdrPlasmaAir3Bourdon::PhotonOne* Photon1 =
    static_cast<CdrPlasmaAir3Bourdon::PhotonOne*>(&(*m_rtSpecies[m_pho1_idx]));
  const CdrPlasmaAir3Bourdon::PhotonTwo* Photon2 =
    static_cast<CdrPlasmaAir3Bourdon::PhotonTwo*>(&(*m_rtSpecies[m_pho2_idx]));
  const CdrPlasmaAir3Bourdon::PhotonThree* Photon3 =
    static_cast<CdrPlasmaAir3Bourdon::PhotonThree*>(&(*m_rtSpecies[m_pho3_idx]));

  const Real Sph =
    m_photoi_eff * Units::c * m_O2frac * m_p *
    (Photon1->get_A() * a_Photon_densities[m_pho1_idx] + Photon2->get_A() * a_Photon_densities[m_pho2_idx] +
...
Vector<Real>
CdrPlasmaAir3Bourdon::computeCdrDiffusionCoefficients(const Real         a_time,
                                                      const RealVect     a_pos,
                                                      const RealVect     a_E,
...
Vector<RealVect>
CdrPlasmaAir3Bourdon::computeCdrDriftVelocities(const Real         a_time,
                                                const RealVect     a_pos,
                                                const RealVect     a_E,
                                                const Vector<Real> a_cdr_densities) const
{
...
Vector<Real>
CdrPlasmaAir3Bourdon::computeCdrDomainFluxes(const Real           a_time,
                                             const RealVect       a_pos,
                                             const int            a_dir,
...
Vector<Real>
CdrPlasmaAir3Bourdon::computeCdrElectrodeFluxes(const Real         a_time,
                                                const RealVect     a_pos,
...
Vector<Real>
CdrPlasmaAir3Bourdon::computeCdrDielectricFluxes(const Real         a_time,
                                                 const RealVect     a_pos,
                                                 const RealVect     a_normal,
                                                 const RealVect     a_E,
                                                 const Vector<Real> a_cdr_densities,
...
Vector<Real>
CdrPlasmaAir3Bourdon::computeCdrFluxes(const Real         a_time,
                                       const RealVect     a_pos,
                                       const RealVect     a_normal,
                                       const RealVect     a_E,
                                       const Vector<Real> a_cdr_densities,
...
Real
CdrPlasmaAir3Bourdon::initialSigma(const Real a_time, const RealVect a_pos) const
{
  return 0.0;
}

Real
CdrPlasmaAir3Bourdon::computeAlpha(const Real a_E, const RealVect a_pos) const
{
