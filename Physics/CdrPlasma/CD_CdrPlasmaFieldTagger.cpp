...
void CdrPlasmaFieldTagger::computeElectricField(EBAMRCellData& a_electricField, EBAMRCellData& a_gradElectricField) const {
...
void CdrPlasmaFieldTagger::computeTracers() const {
  ...
  for (int lvl = 0; lvl <= m_amr->getFinestLevel(); lvl++){
	...
	const RealVect gradE = RealVect(D_DECL(gradElectricFieldReg(iv, 0), gradElectricFieldReg(iv, 1), gradElectricFieldReg(iv, 2)));
	...
	const RealVect gradE  = RealVect(D_DECL(gradElectricField(vof, 0), gradElectricField(vof, 1), gradElectricField(vof, 2)));
