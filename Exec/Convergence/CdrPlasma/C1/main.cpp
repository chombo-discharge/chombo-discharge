...
main(int argc, char* argv[])
{
  ...
  for (const auto& cells : nCells) {
    ...
    if (notCoarsest) {
      ...
      const Real Linf = DataOps::norm(*finePhi[0], 0, *amr->getVofIterator("primal", phase::gas)[0]) * 1.E-18;
      const Real L1   = DataOps::norm(*finePhi[0], 1, *amr->getVofIterator("primal", phase::gas)[0]) * 1.E-18;
      const Real L2   = DataOps::norm(*finePhi[0], 2, *amr->getVofIterator("primal", phase::gas)[0]) * 1.E-18;
