...
At the bottom of the V-cycle, the user may specify the desired bottom solver type. For multifluid cases (i.e. with dielectrics), we only have relaxations solvers on the bottom. To use these solvers, the user must set **gmg_bottom_solver** to *simple*. For single-fluid cases (i.e. geometries with only electrodes), the user can use a more exact biconjugate stabilized gradient solver. To use this solver, set **gmg_bottom_solver** to *bicgstab*. It (almost) always pays off to use the *bicgstab* solver on the V-cycle bottom since it is more exact than the relaxation sweep. 
