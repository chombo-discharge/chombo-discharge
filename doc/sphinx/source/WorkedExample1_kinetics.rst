...
Source terms for CDR equations
******************************

The source term are next computed, but is set to zero here. 

.. literalinclude:: links/worked_example1_kinetics.H
   :language: c++
   :lines: 30-39

Boundary conditions on electrodes and insulators
************************************************

The definition of the electrode and dielectric flux function are the same; it uses an outflow boundary condition, setting the flux equal to the extrapolated flux from the interior of the domain. Note that, in PlasmaC, a positive flux is directed out of the domain. 

.. literalinclude:: links/worked_example1_kinetics.H
   :language: c++
   :lines: 40-53

Source terms for RTE equations
******************************

Since we don't have any RTE solvers for this case, the RTE source terms can be set to something arbitrary (zero here). 
...
**********************
Finally, the initial surface charge density is also set to zero
