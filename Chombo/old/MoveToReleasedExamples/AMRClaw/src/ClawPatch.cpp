...
ClawPatch::~ClawPatch()
{
    ...
    if ( m_auxtype_int != NULL )
    {
      delete [] m_auxtype_int;
      m_auxtype_int = NULL;
    }
}
...
void ClawPatch::get_inputParams()
{
  ...
  int mx, my;
  if (!ppclaw.query("mx",mx))
  {
      MayDay::Error("Input variable claw.mx missing");
 }
  if (!ppclaw.query("my",my))
  {
      MayDay::Error("Input variable claw.my missing");
  }

#if CH_SPACEDIM == 3
  int mz;
  if (!ppclaw.query("mz",mz))
  {
      MayDay::Error("Input variable claw.mz missing");
  }
#endif

  Real initial_dt;
  if (!ppclaw.query("initial_dt",initial_dt)) initial_dt = -1;

  Real max_cfl;
  if (!ppclaw.query("max_cfl",max_cfl))
  {
      MayDay::Error("Input claw.max_cfl missing");
  }


  // Method parameters start here...
  // method(2:3) : order of integration
  std::vector<int> order(SpaceDim,0);
  if (!ppclaw.queryarr("order",order,0,SpaceDim))   // method(2)
    {
      MayDay::Error("Input claw.order missing, or not enough entries specified.");
    }


  // method(5) : src term splitting
  int src;
  if (!ppclaw.query("src",src)) src = 0;

  // method(6) : mcapa
  int mcapa;
  if (!ppclaw.query("mcapa",mcapa)) mcapa = 0;

  // method(7) : maux
  int maux;
  if (!ppclaw.query("maux",maux)) maux = 0;

  std::vector<std::string> auxtype;
  if (maux > 0)
  {
      if (!ppclaw.queryarr("auxtype",auxtype,0,maux))
      {
          MayDay::Error("Input variable claw.auxtype missing, or not enough entries specified");
      }
  }

  int meqn;
  if (!ppclaw.query("meqn",meqn))
    {
      MayDay::Error("Input variable claw.meqn missing.");
    }

  int mwaves;
  if (!ppclaw.query("mwaves",mwaves))
    {
      MayDay::Error("Input variable claw.mwaves missing");
    }

  std::vector<int> mthlim(mwaves,0);
  if (!ppclaw.queryarr("mthlim",mthlim,0,mwaves))
  {
      MayDay::Error("Input variable claw.mthlim missing");
  }

  int mbc;
  if (!ppclaw.query("mbc",mbc))
    {
      MayDay::Error("Input variable claw.mbc missing");
    }


  // This has already been read, but I just need it for checking against mthbc
  // Determine which spatial directions are periodic

  std::vector<int> mthbc(2*SpaceDim,0);
  if (!ppclaw.queryarr("mthbc",mthbc,0,2*SpaceDim))
  {
      MayDay::Error("Input variable claw.mthbc missing, or not enough entries specified.");
  }

  for (int i = 0; i < SpaceDim; i++)
  {
      if ( (mthbc[2*i] == 2 && mthbc[2*i+1] != 2) ||
           (mthbc[2*i] != 2 && mthbc[2*i+1] == 2))
      {
          MayDay::Error("Problem with specification of periodic boundary conditions in claw.mthbc.");
      }
  }


  std::vector<std::string> stateNames;
  if (!ppclaw.queryarr("stateNames",stateNames,0,meqn))
  {
      MayDay::Error("Input variable claw.stateNames missing, or not enough entries specified");
  }


  Real xlower;
  if (!ppclaw.query("xlower",xlower)) xlower = 0;

  Real ylower;
  if (!ppclaw.query("ylower",ylower)) ylower = 0;

#if CH_SPACEDIM == 3
  Real zlower;
  if (!ppclaw.query("zlower",zlower)) zlower = 0;
#endif


  Real xupper;
  if (!ppclaw.query("xupper",xupper))
  {
      MayDay::Error("Input variable claw.xupper missing");
  }

  Real yupper;
  if (!ppclaw.query("yupper",yupper))
  {
      MayDay::Error("Input variable claw.yupper missing");
  }

#if CH_SPACEDIM == 3
  Real zupper;
  if (!ppclaw.query("zupper",zupper))
  {
      MayDay::Error("Input variable claw.zupper missing");
  }
#endif


  int maxlevel;
  if (!ppclaw.query("max_level",maxlevel))
  {
      MayDay::Error("Input variable claw.max_level is missing");
  }
  std::vector<int> refratios(maxlevel,0);
  if (!ppclaw.queryarr("ref_ratio",refratios,0,maxlevel))
  {
      MayDay::Error("Input variable claw.ref_ratio is missing");
  }
  ...
  set_mthlim(mthlim);
  set_mthbc(mthbc);
  set_stateNames(stateNames);
  set_initial_dt(initial_dt);
  if (maux > 0)
  {
      set_auxtype(auxtype);
  }
...
void ClawPatch::print_inputParams()
{
  ...
  pout() << "State names : " << endl;
  for (int i = 0; i < m_meqn; i++)
    {
      pout() << "  " << m_stateNames[i] << endl;
    }
  pout() << endl;
