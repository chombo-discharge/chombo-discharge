...
void GaussianAdvectMultiMappedIBC::initializeWithJ(LevelData<FArrayBox>& a_phi)
{
  initializeUnified(a_phi, true);
}

void GaussianAdvectMultiMappedIBC::initializeUnified(LevelData<FArrayBox>& a_phi,
                                                     bool a_includeJ)
{
...
void
GaussianAdvectMultiMappedIBC::advVel(LevelData<FArrayBox>& a_advVel,
                                     Real a_time)
{
              ...
              for (bit.begin(); bit.ok(); ++bit)
                {
                  ...
                  D_TERM6( advVelFab(iv,0) = L*L*(m_uniformVel[0]-m_uniformVel[1]*osc);,
                           advVelFab(iv,1) = L*L*(m_uniformVel[1]+m_uniformVel[0]*osc);,
                           advVelFab(iv,2) = 0; ,
                           advVelFab(iv,3) = 0; ,
                           advVelFab(iv,4) = 0; ,
                           advVelFab(iv,5) = 0; );
...
// Set boundary fluxes
void GaussianAdvectMultiMappedIBC::primBC(FArrayBox&            a_WGdnv,
...
void GaussianAdvectMultiMappedIBC::setBdrySlopes(FArrayBox&       a_dW,
                                          const FArrayBox& a_W,
                                          const int&       a_dir,
                                          const Real&      a_time)
{
  MayDay::Error("GaussianAdvectMultiMappedIBC::setBdrySlopes not defined");
  // Cartesian version calls FORT_SLOPEBCSF.
}

// Adjust boundary fluxes to account for artificial viscosity
// Do nothing for this problem
void GaussianAdvectMultiMappedIBC::artViscBC(FArrayBox&       a_F,
                                      const FArrayBox& a_U,
