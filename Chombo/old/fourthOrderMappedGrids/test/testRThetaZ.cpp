           ...
           const FourthOrderCoordSys* a_coordsys)
{
  ...
  // if verbose, compute errors and convergene rates for F
  if (verbose)
    {
      for (dit.begin(); dit.ok(); ++dit)
        {
...
writeMeshToFile(char* a_filestr,
                CoordSys<FArrayBox, FluxBox>* a_coordSysPtr,
          ...
          for (int j=jlo; j<=jhi; j++)
            {
              ...
              RealVect x = a_coordSysPtr->realCoord(loc);
...
testRThetaZ()
{
  ...
  RThetaZCoordSysFactory factory(baseDomain, nRefVect, dxVect);
  ...
  factory.m_volInterval = volInterval;
  ...
  for (int level = 0; level<numLevels; level++)
    {
      ...
      CoordSys<FArrayBox, FluxBox>* levelCoordSys = factory.getCoordSys(levelGrids,
                                                                        levelDomain,
                                                                        ghostVect);
      ...
      if (levelCoordSys == NULL)
        {
          returnCode += 1;
          pout() << "RThetaZCoordSysFactory::getCoordSys FAILED" << endl;
        }
      else
        {
            ...
            if (maxErr > eps)
              {
                // fail
                returnCode += 10;
                pout() << "Invertability test FAILS"
                       << " -- Max(error) = " << maxErr << endl;
              }
          } // end invertability test
          ...
          FourthOrderCoordSys* FOCS = dynamic_cast<FourthOrderCoordSys*>(levelCoordSys);
          {
            ...
            // print N_11
            {
               ...
               if (writeN11)
                 {
                   int compN11 = FOCS->getMetricTermComponent(0,0);
                   for (dit.begin(); dit.ok(); ++dit)
                   {
                     const FluxBox & thisN = metricTerms[dit];

                     for (int dir=0; dir<SpaceDim; dir++)
                     {
                       const FArrayBox & thisNDir = thisN[dir];

                       cout << "N(1,1) in direction " << dir << " is:" << endl;
                    ...
                    for (int dir = 0; dir<SpaceDim; dir++)
                      {
                        for (int comp=0; comp<SpaceDim; comp++)
                          {
                            int Ncomp = FOCS->getMetricTermComponent(comp,dir);
          ...
          LevelData<FArrayBox> cellVolErr(cellVol.getBoxes(),
                                          cellVol.nComp());
                    ...
                    for (int sComp=0; sComp<SpaceDim; sComp++)
                      {
                        for (int dDir=0; dDir<SpaceDim; dDir++)
                          {
                            int thisComp = FOCS->getMetricTermComponent(sComp,
                                                                        dDir);
          ...
          initF(F, levelDomain, dxLevel, FOCS);
          ...
          // now evaluate error
          {
            ...
            // along with the cell volume and its errro
            if (writePlotFiles)
              {
                LevelData<FArrayBox> plotData(levelGrids, 4);
                ...
                Interval volInterval(2,2);
                cellVol.copyTo(cellVol.interval(),
                               plotData, volInterval);

                Interval volErrInterval(3,3);
                cellVolErr.copyTo(cellVolErr.interval(),
                                  plotData, volErrInterval);
