...
//     a_x0            - Location of lower left corner, usually zero
//     a_center        - Location of center of top hat
//     a_r0            - Full width of top hat
void TopHatIBC::setParams(Real a_r0, RealVect a_center, RealVect a_x0)
{
...
void
TopHatIBC::setUniformVel(const RealVect& a_vel)
{
  m_velType = uniform;
  m_uniformVel = a_vel;
}
...
void
TopHatIBC::setSolidBodyRotation(const RealVect& a_rotationCenter,
                                const Real a_omega)
{
  m_velType = solidBody;
...
BasicIBC* TopHatIBC::new_basicIBC()
{
  TopHatIBC* retval = new TopHatIBC();
...
void TopHatIBC::initialize(LevelData<FArrayBox>& a_phi,
                           ...
                           Real a_time)
{
  ...
  // commented out by petermc, 17 Aug 2010
  //  fourthOrderAverage(a_phi);
}


void
TopHatIBC::advVel(LevelData<FArrayBox>& a_advVel,
                  const ProblemDomain& a_domain,
                  const CoordSys<FArrayBox,FluxBox>& a_coordSys,
       ...
       for (int dir=0; dir<SpaceDim; dir++)
        {
          if (m_velType == uniform)
            ...
            {
              MayDay::Error("TopHatBC::advVel -- bad velType");
            }
...
void
TopHatIBC::ghostCellBC(LevelData<FArrayBox>& a_phi,
                       const ProblemDomain& a_domain,
                       const CoordSys<FArrayBox,FluxBox>& a_coordSys,
                       Real a_dx,
                       Real a_time)
...
/// Set up initial conditions
void
TopHatIBC::pointVal(FArrayBox& a_phi,
                    const ProblemDomain& a_domain,
                    const CoordSys<FArrayBox,FluxBox>& a_coordSys,
                    const Box& a_box,
                    bool a_includeJ,
