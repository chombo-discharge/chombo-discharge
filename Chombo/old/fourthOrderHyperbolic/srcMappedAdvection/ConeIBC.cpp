...
//     a_r0            - Full width of Cone.
void ConeIBC::setParams(Real a_r0, Real a_mag,
...
void
ConeIBC::setUniformVel(const RealVect& a_vel)
{
  m_velType = uniform;
  m_uniformVel = a_vel;
}
...
void
ConeIBC::setSolidBodyRotation(const RealVect& a_rotationCenter,
                              const Real a_omega)
{
  m_velType = solidBody;
...
BasicIBC* ConeIBC::new_basicIBC()
{
  ConeIBC* retval = new ConeIBC();
...
void ConeIBC::initialize(LevelData<FArrayBox>& a_phi,
                         ...
                         Real a_time)
{
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    pointVal(a_phi[dit], a_domain, a_coordSys,
             a_phi[dit].box(), includeJ, a_dx, a_time);
  }
...
void
ConeIBC::advVel(LevelData<FArrayBox>& a_advVel,
                const ProblemDomain& a_domain,
                const CoordSys<FArrayBox,FluxBox>& a_coordSys,
       ...
       for (int dir=0; dir<SpaceDim; dir++)
        {
          if (m_velType == uniform)
            ...
            {
              MayDay::Error("ConeBC::advVel -- bad velType");
            }
...
void
ConeIBC::ghostCellBC(LevelData<FArrayBox>& a_phi,
                     const ProblemDomain& a_domain,
                     const CoordSys<FArrayBox,FluxBox>& a_coordSys,
                     Real a_dx,
                     Real a_time)
...
/// Set up initial conditions
void
ConeIBC::pointVal(FArrayBox& a_phi,
                  const ProblemDomain& a_domain,
                  const CoordSys<FArrayBox,FluxBox>& a_coordSys,
                  const Box& a_box,
                  bool a_includeJ,
