...
Real computeMappedNewDt(const LevelData<FluxBox>& a_faceVel,
                        const NewFourthOrderCoordSys* a_coordSysPtr,
                        Real a_cfl)
{
  ...
  for (dit.begin(); dit.ok(); ++dit)
    {
      ...
      */
      ...
      a_coordSysPtr->getN(faceMetricTerms, grownBox1);
      ...
      a_coordSysPtr->computeMetricTermProductAverage(faceNormalVelFlub,
                                                     faceVelFlub,
                                                     faceMetricTerms,
                                                     bx,
                                                     doFourthOrder);
      ...
      a_coordSysPtr->cellVol(cellVolumesFab, faceMetricTerms, bx);
