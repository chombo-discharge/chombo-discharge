...
// Null constructor
GaussianAdvectMappedIBC::GaussianAdvectMappedIBC()
{
...
void GaussianAdvectMappedIBC::setParams(Real a_r0,
                                        RealVect a_center,
                                        RealVect a_x0)
{
...
void
GaussianAdvectMappedIBC::setUniformVel(const RealVect& a_vel)
{
  m_velType = uniform;
  m_uniformVel = a_vel;
}
...
void
GaussianAdvectMappedIBC::setSolidBodyRotation(const RealVect& a_rotationCenter,
                                              const Real a_omega)
{
  m_velType = solidBody;
...
void
GaussianAdvectMappedIBC::setTranslatingOscillation(
...
PhysMappedIBC* GaussianAdvectMappedIBC::new_physIBC()
{
  GaussianAdvectMappedIBC* retval = new GaussianAdvectMappedIBC();
...
void GaussianAdvectMappedIBC::initialize(LevelData<FArrayBox>& a_phi)
{
...
void
GaussianAdvectMappedIBC::advVel(LevelData<FArrayBox>& a_advVel)
{
       ...
       for (int dir=0; dir<SpaceDim; dir++)
        {
          if (m_velType == uniform)
            ...
            {
              MayDay::Error("GaussianAdvectMappedIBC::advVel -- bad velType");
            }
...
void
GaussianAdvectMappedIBC::ghostCellBC(LevelData<FArrayBox>& a_phi)
{
...
void
GaussianAdvectMappedIBC::exactSoln(LevelData<FArrayBox>& a_phi)
{
  DataIterator dit = a_phi.boxLayout().dataIterator();
...
/// Set up initial conditions
void
GaussianAdvectMappedIBC::pointVal(FArrayBox& a_phi,
                                  const Box& a_box,
                                  bool a_includeJ)
{
...
// Set boundary fluxes
void GaussianAdvectMappedIBC::primBC(FArrayBox&            a_WGdnv,
...
void GaussianAdvectMappedIBC::setBdrySlopes(FArrayBox&       a_dW,
                                            const FArrayBox& a_W,
                                            const int&       a_dir,
                                            const Real&      a_time)
{
  MayDay::Error("GaussianAdvectMappedIBC::setBdrySlopes not defined");
  // Cartesian version calls FORT_SLOPEBCSF.
}

// Adjust boundary fluxes to account for artificial viscosity
// Do nothing for this problem
void GaussianAdvectMappedIBC::artViscBC(FArrayBox&       a_F,
                                        const FArrayBox& a_U,
