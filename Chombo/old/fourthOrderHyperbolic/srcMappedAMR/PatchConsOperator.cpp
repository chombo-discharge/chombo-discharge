...
PatchConsOperator::PatchConsOperator(const int a_numFluxesPerField)
  :
...
void
PatchConsOperator::define(const ProblemDomain&    a_domain,
                          const Real&             a_dx,
...
void PatchConsOperator::setCurrentTime(const Real& a_currentTime)
{
  ...
  m_molPhysics->setCurrentTime(a_currentTime);
}

//////////////////////////////////////////////////////////////////////////////

void
PatchConsOperator::setCurrentBox(const DataIndex& a_currentDataIdx,
...
//void
//PatchConsOperator::setCurrentCoordSys(const NewCoordSys* a_coordSys)
//{
//  m_molPhysics->setCurrentCoordSys(a_coordSys);
//}
...
void
PatchConsOperator::evalFlux(const FArrayBox& a_UavgFab,
                            FluxBox&         a_FfaceAvg,
...
void PatchConsOperator::evalRHS(FArrayBox&          a_LofU,
...
void
PatchConsOperator::getAllFluxes(FluxBox&        a_FfaceAvg,  // On box+1
          ...
          for (int jdir = 0; jdir < SpaceDim; jdir++)
            {
              ...
              m_molPhysics->getFlux(FfaceAvgDirD,
                                    WfaceAvgDir,
                                    jdir,
                                    faceBox1);
            }
              ...
              for (int jdir = 0; jdir < SpaceDim; jdir++)
                {
                  ...
                  m_molPhysics->getFlux(FfaceCenDirD,
                                        WfaceCenDir,
                                        jdir,
                                        faceBox0);
                }
...
void
PatchConsOperator::forwardBasisTransform(FArrayBox& a_W,
                                         const int a_dir,
                                         const Box& a_box)
{
...
void
PatchConsOperator::reverseBasisTransform(FArrayBox& a_W,
                                         const int a_dir,
                                         const Box& a_box)
{
  CH_assert(m_unitNormalLayPtr != NULL);
                                          ...
                                          Real             a_weight)
{
  ...
  Box bx1inDomain = grow(m_currentBox, 1);
  bx1inDomain &= m_blockDomain;
  a_flux.resize(m_currentBox, m_numFluxes);
...
void
PatchConsOperator::addMappedArtificialViscosity(FArrayBox&       a_JUnewFab,
                                                const FArrayBox& a_UoldFab,
                                                FluxBox&         a_flux,
                                                const FluxBox&   a_N,
                                                const FArrayBox& a_J,
                                                ...
                                                Real             a_weight)
{
  ...
  bx1inDomain &= m_blockDomain;
  FArrayBox divFlux(a_box, m_molPhysics->numFluxes());
  divFlux.setVal(0.);

  m_molPhysics->mappedArtVisc(a_flux,
                              a_UoldFab,
                              a_N,
                              a_J,
                              (*m_unitNormalLayPtr)[m_currentDataIdx],
                              m_artificialViscosity,
                              m_currentTime,
                              a_box);
