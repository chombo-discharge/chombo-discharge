...
//
// $Id: ABecLaplacian.cpp,v 1.2 2007-07-24 01:14:43 ligocki Exp $
//
#include <winstd.H>
...
ABecLaplacian::ABecLaplacian (const BndryData& _bd,
    ...
    beta(beta_def)
{
  CH_TIME("CCSE::ABecLapacian::ABecLaplacian1");
...
ABecLaplacian::ABecLaplacian (const BndryData& _bd,
    ...
    beta(beta_def)
{
  CH_TIME("CCSE::ABecLapacian::ABecLaplacian2");
    initCoefficients(_bd.boxes());
}

ABecLaplacian::~ABecLaplacian ()
{
  CH_TIME("CCSE::ABecLapacian::~ABecLaplacian");
    clearToLevel(-1);
}
...
{
  CH_TIME("CCSE::ABecLapacian::norm");
  BL_ASSERT(nm == 0);
...
{
  CH_TIME("CCSE::ABecLapacian::clearToLevel");
    BL_ASSERT(level >= -1);
...
{
  CH_TIME("CCSE::ABecLapacian::prepareForLevel");
...
{
  CH_TIME("CCSE::ABecLapacian::initCoefficients");
    const int nComp=1;
    const int nGrow=0;
    acoefs.resize(1);
    bcoefs.resize(1);
...
{
  CH_TIME("CCSE::ABecLapacian::invalidate_a_to_level");
    lev = (lev >= 0 ? lev : 0);
...
{
  CH_TIME("CCSE::ABecLapacian::invalidate_b_to_level");
    lev = (lev >= 0 ? lev : 0);
    for (int i = lev; i < numLevels(); i++)
        b_valid[i] = false;
}
...
{
  CH_TIME("CCSE::ABecLapacian::compFlux");
    int level = 0;
    int src_comp = 0;
    int num_comp = 1;
    if (do_ApplyBC)
...
ABecLaplacian::Fsmooth (MultiFab&       solnL,
                        ...
                        int             redBlackFlag)
{
  CH_TIME("CCSE::ABecLapacian::Fsmooth");
...
ABecLaplacian::Fapply (MultiFab&       y,
                       ...
                       int             level)
{
  CH_TIME("CCSE::ABecLapacian::Fapply");
    const BoxArray& bxa = gbox[level];
    const MultiFab& a   = aCoefficients(level);
