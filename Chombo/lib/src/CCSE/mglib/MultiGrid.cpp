//
// $Id: MultiGrid.cpp,v 1.2 2007-07-24 01:14:43 ligocki Exp $
// 
#include <winstd.H>
...
{
  CH_TIME("CCSE::MultiGrid::initialize");
    ParmParse pp("mg");
...
MultiGrid::MultiGrid (LinOp &_Lp)
    ...
    Lp(_Lp)
{
  CH_TIME("CCSE::MultiGrid::MultiGrid");
    if (!initialized)
        initialize();
...
{
  CH_TIME("CCSE::MultiGrid::errorEstimate");
...
{
  CH_TIME("CCSE::MultiGrid::prepareForLevel");
    //
...
MultiGrid::residualCorrectionForm (MultiFab&       resL,
                                   ...
                                   int             level)
{
  CH_TIME("CCSE::MultiGrid::residualCorrectionForm");
    //
...
{
  CH_TIME("CCSE::MultiGrid::numLevels");
    int ng = Lp.numGrids();
    int lv = numLevelsMAX;
    //
...
MultiGrid::coarsestSmooth (MultiFab&      solL,
                           ...
                           int            local_usecg)
{
  CH_TIME("CCSE::MultiGrid::coarsestSmooth");
...
MultiGrid::average (MultiFab&       c,
                    const MultiFab& f)
{
  CH_TIME("CCSE::MultiGrid::average");
    //
...
MultiGrid::interpolate (MultiFab&       f,
                        const MultiFab& c)
{
  CH_TIME("CCSE::MultiGrid::interpolate");
    //
