...
int makeGeometry(Box& a_domain)
{
  ...
  if (whichgeom == 0)
  ...
  else if (whichgeom == 1)
    {
      ...
      GeometryShop workshop(ramp,RealVect::Zero, dxVect[0], a_domain);
  ...
  else if (whichgeom == 5)
    {
      ...
      GeometryShop workshop(implicit,RealVect::Zero, dxVect[0], a_domain);
      //this generates the new EBIS
      ebisPtr->define(a_domain, origin, dx, workshop);
    }
  ...
  for (int ilev = 0; ilev < ebisPtr->numLevels(); ilev++)
    {
      ...
      eekflag = checkLevel(   levelEBISL, levelDBL, levelDomain, levelDx, ilev);
      if (eekflag != 0)
        {
