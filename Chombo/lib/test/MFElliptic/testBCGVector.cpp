...
const Real pi = 2*atan2((Real)1,(Real)0);

/// Global variables for handling output:
static const char* pgmname = "testBCGVector" ;
static const char* indent = "   ";
static const char* indent2 = "      " ;


const Real mu_inside = 1;
const Real mu_outside = 1;
const RealVect u_jump(D_DECL(1, -0.5, .33333));
...
#define VEL D_DECL(x*x+y*y, x+1./3.*y*y, a_point[2]*a_point[2]-2*y*y)
#define RHS D_DECL(4,2.0/3.0,-2)

//#define VEL D_DECL(x/2.0, x+y, a_point[2]*a_point[2]+2+y*y)
//#define RHS D_DECL(0,0,-2)
...
main(int argc,char **argv)
{
  ...
  // begin forever present scoping trick
  {
    ...
    Vector<Real> coveredVal(CH_SPACEDIM,7.0);
...
//     outsideBC->setValue(0.0);
    ...
    //make data holders
    Vector<int> comps(2,CH_SPACEDIM);
    int ghost = 3;
    ...
    while (step < steps)
    {
      ...
      MFCellFactory fineFactory(*mfIndexSpace, grids[2],domainFine,
                                comps, ghost);
      mfvector[2]->define(grids[2], CH_SPACEDIM, ghost*IntVect::Unit, fineFactory);
      rhsvector[2]->define(grids[2], CH_SPACEDIM, IntVect::Zero, fineFactory);
      levelset[2]->define(grids[2], 1, IntVect::Unit);

      MFCellFactory  medFactory(*mfIndexSpace, grids[1], domainMedi,
                                comps, ghost);
      mfvector[1]->define(grids[1], CH_SPACEDIM, ghost*IntVect::Unit, medFactory);
      rhsvector[1]->define(grids[1], CH_SPACEDIM, IntVect::Zero, medFactory);
      levelset[1]->define(grids[1], 1, IntVect::Unit);

      MFCellFactory coarseFactory(*mfIndexSpace, grids[0],domainCoar,
                                  comps, ghost);
      mfvector[0]->define(grids[0], CH_SPACEDIM, ghost*IntVect::Unit, coarseFactory);
      rhsvector[0]->define(grids[0], CH_SPACEDIM, IntVect::Zero, coarseFactory);
      ...
      for (int i=0; i<3; i++)
      {
        ...
        op.define(*mfIndexSpace, CH_SPACEDIM, grids[i], dumdbl, false, true, dumdbl, dumdbl,
                  xVec[i], 2, 2,pd[i], bc, 3*IntVect::Unit, IntVect::Zero, false, false,
                  alpha, beta);
//        op.define(*mfIndexSpace, CH_SPACEDIM, grids[i],
//                  xVec[i], pd[i], bc);


        op.setJump(u_jump, RealVect::Zero);
        op.m_relax = 0;
        BiCGStabSolver<LevelData<MFCellFAB> > solver;
        ...
        pout()<<indent<<"Level "<<i<<" Residual L2 norm "<<r2norm<<"  Residual max norm = "
              <<r0norm<<std::endl;
...
//               <<r0norm<<std::endl;
      ...
      Vector<std::string> names(2*CH_SPACEDIM);
      for (int i=0; i<CH_SPACEDIM; i++)
      {
        names[i]            ="residual0_";   names[i].push_back(i+120);
        names[i+CH_SPACEDIM]="residual1_";   names[i+CH_SPACEDIM].push_back(i+120);
      }
      ...
      for (int i=0; i<CH_SPACEDIM; i++)
      {
        names[i]            ="phi0_";  names[i].push_back(i+120);
        names[i+CH_SPACEDIM]="phi1_";  names[i+CH_SPACEDIM].push_back(i+120);
      }
      writeEBHDF5(iter_str, grids, &phaseA, &phaseB, &levelset, names, domainCoar,
                  dxCoar, 1, step, refRatio, 3, true, coveredVal);
#endif
