...
static const char* pgmname = "testRelaxSolver" ;
...
{
  return 3*a_point[0]*a_point[0] + 2*a_point[1];
  //return a_point[0]*2.0/3.0 + a_point[1]*1.5;
  // return 2*a_point[0];
}
...
//     outsideBC->setValue(0.0);
    ...
    while (step < steps)
    {
      ...
      MFCellFactory fineFactory(*mfIndexSpace, grids[2],domainFine,
                                comps, ghost);
      mfvector[2]->define(grids[2], 1, ghost*IntVect::Unit, fineFactory);
      ...
      MFCellFactory  medFactory(*mfIndexSpace, grids[1], domainMedi,
                                comps, ghost);
      mfvector[1]->define(grids[1], 1, ghost*IntVect::Unit, medFactory);
      ...
      MFCellFactory coarseFactory(*mfIndexSpace, grids[0],domainCoar,
                                  comps, ghost);
      mfvector[0]->define(grids[0], 1, ghost*IntVect::Unit, coarseFactory);
...
//           setValue(phase1, OutsideDirichletBC(),pd[lev].domainBox()  , xVec[lev], origin);
      ...
      for (int i=0; i<3; i++)
      {
        ...
        LevelData<MFCellFAB> correction;
        ...
        op.define(*mfIndexSpace, 1, grids[i], dumdbl, false, true, dumdbl, dumdbl,
                  ...
                  alpha, beta);
//        op.define(*mfIndexSpace, 1, grids[i],
//                  xVec[i], pd[i], bc);


        RelaxSolver<LevelData<MFCellFAB> > solver;
