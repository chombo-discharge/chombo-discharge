...
public:
    WeightedBoxList (std::list<WeightedBox>* lb)
      :
      m_lb(lb),
      m_weight(0)
    {
    ...
    std::list<WeightedBox>::const_iterator begin () const
    {
      return m_lb->begin();
    }

    std::list<WeightedBox>::iterator begin ()
    {
      return m_lb->begin();
    }

    std::list<WeightedBox>::const_iterator end () const
    {
      return m_lb->end();
    }

    std::list<WeightedBox>::iterator end ()
    {
      return m_lb->end();
    }
...
knapsack (const std::vector<long>& pts, int nprocs)
{
    ...
    CH_ASSERT(int(wblq.size()) == nprocs);
    for (unsigned int i = 0; i < pts.size(); ++i)
    {
    ...
    CH_ASSERT(int(wblqg.size()) == nprocs);
    wblqg.sort();
    //
...
MinimizeCommCosts (std::vector<int>&        procmap,
                   ...
                   int                      nprocs)
{
    CH_PROFILE("MinimizeCommCosts()");

    CH_ASSERT(ba.size() == pts.size());
    CH_ASSERT(procmap.size() >= ba.size());
