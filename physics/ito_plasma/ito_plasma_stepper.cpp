...
void ito_plasma_stepper::compute_E(EBAMRIVData& a_E_eb,  const phase::which_phase a_phase, const EBAMRCellData& a_E_cell){
  ...
  const irreg_amr_stencil<EbCentroidInterpolationStencil>& interp_stencil = m_amr->getEbCentroidInterpolationStencilStencils(m_fluid_realm, a_phase);
...
void ito_plasma_stepper::compute_ito_diffusion_lfa(Vector<EBAMRCellData*>&       a_diffco_funcs,
  ...
  for (auto solver_it = m_ito->iterator(); solver_it.ok(); ++solver_it){
    ...
    if(solver->is_diffusive()){
      ...
      m_amr->averageDown(*a_diffco_funcs[idx], m_particle_realm, m_phase);
      m_amr->interpGhost(*a_diffco_funcs[idx], m_particle_realm, m_phase);
#endif
