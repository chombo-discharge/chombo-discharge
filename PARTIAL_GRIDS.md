...
## Where the branch stands (2026-09-17)

The builder is complete and runs. `makeGrids` builds the six steps as planned; `PolyhedralGeometryShop.write_stl`
reconstructs the surface on the builder's grids and writes it per phase and per level; under
`geometry_generation = polyhedral` the index space is generated by `PolyhedralGeometryShop` over
`ScanShop`'s full grids, as in #730 -- the builder's grids drive only the surface until #732 consumes
them. The seam machinery (`restrictFace`, `closeInterface`, `mergeCoplanar`) is back on this branch by
patch from the original commits. PR #731's note was rewritten for this scope.

**Verified on `Exec/Examples/Electrostatics/ProfiledSurface`, 3-D, geometry only, 16 ranks.** Depth 1: 141744
gas and 94444 solid triangles, closed except at the domain floor. Depth 3, `refine_angles = 45`: 1.84 M gas
and 1.49 M solid triangles over levels 6 and 7 (starting level 4, 108 boxes; level 7, 3330 boxes of which
2188 tiles), closed except for two edges written from both sides one ulp apart. No abort in either.

**What those runs found, in order.**

1. 70920 open edges between cells that both wrote the shared vertex -- from their own centres, which round
   differently on the two sides. Facet coordinates are now formed from the cell index:
   `probLo + dx · (iv + (local + 0.5))` is the same double from both cells for anything on a common face or
   edge crossing.
2. Four cells refused to close after restriction. `restrictFace` takes the `2^(D-1)` children *on the face
   in quadrant order* and was handed all `2^D` children by corner bits, so an upper face read the lower
   children's crossings. The multichord trap was rightly silent: the coarse edges were single-crossed.
3. With that fixed, 2824 open edges, every one a T-junction: the collinear pass in `mergeCoplanar` merged
   two children's chord segments where the surface is flat across them, dropping the vertex the finer cells
   share. The multichord seam carries that vertex by construction -- the union of the children's face
   polygons has it -- and the cleanup removed it. A chord vertex on a child boundary (face-tangential
   coordinate exactly zero, since the children map with origin `±0.25`) is now kept; a vertex on the face's
   own boundary is still dropped, because keeping it splits an edge the neighbouring face carries whole and
   `closeInterface` then reads the face boundary as open -- which is what the first, too-broad version of
   the fix did, and ran the polygon store out. The vertex cap is twenty, as #732 measured.
4. The two remaining edges: a child's vertex mapped through `origin + 0.5 · local` at the coarse spacing
   versus through its own index at the fine spacing, one ulp apart. Not chased; a quantisation snap is the
   thing that manufactures cracks.

**Facts the runs established.** `restrictFace`'s child convention is quadrant order on the face, not the
cell's corner bits. The collinear cleanup and the seam are in tension and the vertex position is what
separates their cases. Every irregular box on the start level split at both angles, because a 16-cell box
at `dx = 1.67 mm` is 27 mm wide and the dielectric's flat walls are 20 mm tall -- the first coarse leaves,
and so the first seams, appeared only on level 6. `EBIndexSpace` under `polyhedral` is untouched by any of
this.

## The BVH: plan

**What it is for.** The builder's lists answer for the levels they hold. The tree answers for any domain --
the transient levels above the stop domain, the shop's `InsideOutside` for a box it was not handed, a fill's
ring cell, the ancestor rule -- through two queries, from the section "What the index space asks":
`classify(box, domain)` (Irregular if any piece of the box resolves to an irregular box, else one sign of
`f`; conservative over straddling) and `ancestor(cell, domain)` (the deepest stored box containing the
cell, with its level and both tags).

**`EBGeometry::BVH::PackedBVH<T, P, K, Storage>`** (`Submodules/EBGeometry/Source/EBGeometry_BVH.hpp`), as
preferred for its SIMD traversal. Read from source:

- Built directly from a flat `std::vector<std::pair<P, BV>>` -- bottom-up along a Morton curve with a
  target leaf size (line 1504), top-down with a partitioner, or ClusterSAH -- with no `TreeBVH` in between.
  `BV` is `AABBT<T>`; `T` is the floating type and the SIMD path exists for `K = 4` or `8` with `T = double`
  (AVX; two passes at `K = 8`).
- `Storage` governs only the primitive array: `SharedPtrStorage<P>` (default) or `ValueStorage<P>`, a
  contiguous array of `P` by value.
- Two traversals. `traverse(leafEvaluator, prunePredicate(Node, NodeKey), childOrderer, nodeKeyFactory)` is
  the generic, scalar one: any node predicate. `pruneTraverse(point, state, evalLeaf, pruneDist2)` is the
  SIMD one: it visits the nodes whose bounding box lies within the running squared distance the caller
  returns from `state`, calls `evalLeaf(state, offset, count)` on each leaf reached, and prunes the rest.
  The caller owns the state and the distance bound; nothing about nearest-surface is built in.
- `refit(bvConstructor)` rebuilds bounding volumes in place after primitives move, without changing the
  tree.

**Primitives: the ownership partition, in one coordinate system.** Per the earlier discussion, the tree
holds not the nested boxes but the *pieces* each box owns -- box minus what the next finer level covers,
decomposed in tile coordinates -- so that the pieces tile the domain exactly once and every cell has one
owner, which is its deepest stored box. Then `ancestor` returns exactly one primitive and `classify` is a
fold over the primitives a box overlaps, with no level arithmetic in either. The pieces come from the
records step 3 already keeps (which tiles each box hosts) plus the untouched boxes and, above the start
level, the boxes' own irregular tiles; a partially split box contributes its uncovered remainder. Each
primitive carries the piece box, its level, and its two classifications; its AABB is the piece scaled to
the stop domain's index space, half-open (`{lo, hi + 1}` before scaling, so that `AABBT::intersects`,
which is strict, equals `Box::intersectsNotEmpty`), in doubles, which is exact for every stored level and
for the power-of-two fractions a finer query produces.

**Queries through `pruneTraverse`, so the SIMD path is what runs.**

- `ancestor(cell, domain)`: scale the cell's centre to the stop domain's index space; `pruneTraverse` with
  the pruning bound held at zero -- a node is visited only if its box is at distance zero from the point,
  i.e. contains it -- and the leaf evaluator tests the point against each primitive's half-open box. One
  hit by construction; more than one, or none, is an abort.
- `classify(box, domain)`: scale the box; `pruneTraverse` from its centre with the bound at the squared
  half-diagonal, which is a necessary condition for a node to intersect the box; the leaf evaluator applies
  the exact `intersects` and folds: any Irregular primitive → Irregular, all Regular → Regular, all
  Covered → Covered, a mix of Regular and Covered → Irregular (cannot happen with the ghost margin;
  treated as the conservative answer). A stored box asked about itself resolves to its own pieces.
- Both are `const` and reentrant; the tree is read-only after the build.

**One tree per phase, or one tree with two tags?** The pieces are the same for both phases -- the
hierarchy is shared -- so one tree with both classifications on the primitive is the natural choice and
halves the build. A query takes the phase as an argument and reads that tag.

**Build.** After step 5, from the final lists: form the pieces, scale, and hand the flat list to the Morton
constructor with a target leaf size of a few primitives (to be measured; the SIMD child test favours
`K = 4` or `8`). Rank-identical input gives rank-identical trees. Rebuilt whenever the lists change,
which is once per `makeGrids`; a transient level above the stop domain does not enter the tree -- it is
what the tree answers for.

**Open decisions, to be played as they come.**

- The primitive type. `ValueStorage<P>` wants a small value type: `{Box, int level, InOut gas, InOut
  solid}`. That is a new struct under the data-structures rule -- a genuine domain object (a stored box
  with its level and classifications), not a rename -- to be proposed before it is written. The
  alternative is `P = int` into flat parallel `Vector`s, which keeps the rule trivially satisfied at the
  cost of one indirection per hit and a `shared_ptr`-free but index-based primitive.
- `T = double` or `float`. Doubles keep the stop-domain coordinates exact (integers up to `2^53`); floats
  halve the SoA cache and double the SIMD width but lose exactness above `2^24` cells per direction, which
  is `16M` -- fine for the domains in the record, not for `500k^3` scaled from a coarse level. Doubles
  unless measured otherwise.
- Whether the partition step (box minus its tiles, per box) is worth doing for the tree when decimation has
  already done it for the regular and covered boxes: the two computations coincide for those, and differ
  only for irregular tiles partly covered by finer tiles. Reusing decimation's remainders and adding the
  irregular pieces may be the whole build.
- Where the tree lives: a member of `ComputationalGeometry` built at the end of `makeGrids`, with
...
`PolyhedralGeometryShop.write_stl` is set. `sanityCheck` (on by default, `PolyhedralGeometryShop.sanity_check`) states the
