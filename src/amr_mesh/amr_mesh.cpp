...
void amr_mesh::allocate(MFAMRIVData& a_data, const int a_ncomp, const int a_ghost){
  ...
  for (int lvl = 0; lvl <= m_finest_level; lvl++){
    ...
    if(!ebis_gas.isNull()) ebisl[phase::gas]   = m_realm->get_ebisl(phase::gas)[lvl];
    if(!ebis_sol.isNull()) ebisl[phase::solid] = m_realm->get_ebisl(phase::solid)[lvl];
...
void amr_mesh::reallocate(EBAMRBool& a_data, const std::string a_realm, const int a_ncomp, const int a_lmin){
  CH_TIME("amr_mesh::reallocate(ebamrbool, realm, comp, lmin)");
  if(m_verbosity > 5){
    pout() << "amr_mesh::reallocate(ebamrbool, realm, comp, lmin)" << endl;
  }

  if(!this->query_realm(a_realm)) {
    std::string str = "amr_mesh::reallocate(ebamrbool, realm, phase, lmin) - could not find realm '" + a_realm + "'";
    MayDay::Abort(str.c_str());
  }
...
void amr_mesh::allocate_interface(EBAMRIVData& a_data, const phase::which_phase a_phase, const int a_ncomp, const int a_ghost){
...
void amr_mesh::conservative_average(EBAMRIVData& a_data, phase::which_phase a_phase){
  CH_TIME("amr_mesh::conservative_average(ebamriv, phase)");
  if(m_verbosity > 3){
...
void amr_mesh::interp_ghost(EBAMRCellData& a_data, phase::which_phase a_phase){
  CH_TIME("amr_mesh::interp_ghost(ebamrcell, phase)");
  if(m_verbosity > 3){
    pout() << "amr_mesh::interp_ghost(ebamrcell, phase)" << endl;
  }
...
void amr_mesh::interp_ghost_quad(EBAMRCellData& a_data, phase::which_phase a_phase){
  CH_TIME("amr_mesh::interp_ghost_quad(ebamrcell, phase)");
  if(m_verbosity > 3){
...
void amr_mesh::interp_ghost_pwl(EBAMRCellData& a_data, phase::which_phase a_phase){
  CH_TIME("amr_mesh::interp_ghost_pwl(ebamrcell, phase)");
  if(m_verbosity > 3){
