...
bool Polygon2D::isPointInsidePolygonWindingNumber(const Vec3<double>& a_point) const noexcept {
  const Vec2<double> p = this->projectPoint(a_point);
...
bool Polygon2D::isPointOnEdge(const Vec2<double>& a_point, const Vec2<double>& a_endPoint1, const Vec2<double>& a_endPoint2, const double a_thresh) const noexcept{
...
bool Polygon2D::isPointOnBoundary(const Vec2<double>& a_point, const double a_thresh) const noexcept {
  const int N = m_points.size();

  bool ret = false;
  for (int i = 0; i < N; i++){
    const Vec2<double>& p1 = m_points[i];
    const Vec2<double>& p2 = m_points[(i+1)%N];
...
Vec2<double> Polygon2D::projectPoint(const Vec3<double>& a_point) const noexcept {
  return Vec2<double>(a_point[m_xDir], a_point[m_yDir]);
}
...
int Polygon2D::computeWindingNumber(const Vec2<double>& P) const noexcept {
  int wn = 0;    // the  winding number counter

  const int N = m_points.size();

  auto isLeft = [](const Vec2<double>& P0, const Vec2<double>& P1, const Vec2<double>& P2){
...
int Polygon2D::computeCrossingNumber(const Vec2<double>& P) const noexcept {
  int cn = 0; 
...
double Polygon2D::computeSubtendedAngle(const Vec2<double>& p) const noexcept {
  double sumTheta = 0.0;
  ...
  for (int i = 0; i < N; i++){
    const Vec2<double> p1 = m_points[i]       - p;
    const Vec2<double> p2 = m_points[(i+1)%N] - p;
