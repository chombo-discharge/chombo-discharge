...
bool VoFUtils::isQuadrantWellDefined(const RealVect a_normal){
  ...
  for (int dir = 0; dir < SpaceDim; dir++){
    if (std::abs(v[dir]) == 1.0) ret = false;
  }

  return ret;
}

int VoFUtils::getCardinalDirection(const RealVect a_normal){
  int ret = -1;
  
  const RealVect v = a_normal/a_normal.vectorLength(); // Idiot guard.
  
  for (int dir = 0; dir < SpaceDim; dir++){
    if (std::abs(v[dir]) == 1.0) {
      ret = int(v[dir]);
    }
  }
...
Vector<VolIndex> VoFUtils::getAllVoFsInQuadrant(const VolIndex& a_startVoF, const EBISBox& a_ebisbox, const RealVect a_normal, const int a_radius, const bool a_addStartVoF) {
  ...
  } // Should be able to remove this after we've done the "symmetric quadrant"
  else{
    IntVect quadrant;
    for (int dir = 0; dir < SpaceDim; dir++){
      if(a_normal[dir] <= 0.){
...
Vector<VolIndex> getAllVoFsSymmetric(const VolIndex& a_startVoF, const EBISBox& a_ebisbox, const int a_normal, const int a_radius, const bool a_addStartVoF){
  Vector<VolIndex> ret;
  ...
  // Get all VoFs that can be reached with a monotone path and add them if they fall within the above box.
  Vector<VolIndex> monoVoFs;
  EBArith::getAllVoFsInMonotonePath(monoVoFs, a_startVoF, a_ebisbox, a_radius);
  for (auto& ivof : monoVoFs.stdVector()){
    if(bx.contains(ivof.gridIndex()) && ivof != a_startVoF){
