...
namespace ChomboDischarge {
  ...
  void DataOps::averageCellToFaceAllComps(EBAMRFluxData& a_face_data,
    ...
    for (int lvl = 0; lvl < a_face_data.size(); lvl++){
      DataOps::averageCellToFace_allcomps(*a_face_data[lvl], *a_cell_data[lvl], a_domains[lvl]);
    }
  }
  ...
  void DataOps::dotProd(MFAMRCellData& a_result, const MFAMRCellData& a_data1, const MFAMRCellData& a_data2){
    for (int lvl = 0; lvl < a_result.size(); lvl++){
      DataOps::dotProd(*a_result[lvl], *a_data1[lvl], *a_data2[lvl]);
    }
  }

  void DataOps::dotProd(LevelData<MFCellFAB>& a_result, const LevelData<MFCellFAB>& a_data1, const LevelData<MFCellFAB>& a_data2){
	...
	DataOps::dotProd(result_phase, data1_phase, data2_phase, box);
      }
    }
  }

  void DataOps::dotProd(EBAMRCellData& a_result, const EBAMRCellData& a_data1, const EBAMRCellData& a_data2){
    for (int lvl = 0; lvl < a_result.size(); lvl++){
      DataOps::dotProd(*a_result[lvl], *a_data1[lvl], *a_data2[lvl]);
    }
  }

  void DataOps::dotProd(LevelData<EBCellFAB>& a_result,
    ...
    for (DataIterator dit = a_result.dataIterator(); dit.ok(); ++dit){
      ...
      const Box& box         = a_result.disjointBoxLayout().get(dit());
      DataOps::dotProd(result, data1, data2, box);
  ...
  void DataOps::dotProd(EBCellFAB& a_result, const EBCellFAB& a_data1, const EBCellFAB& a_data2, const Box& a_box){
    const int nc = a_data1.nComp();
  ...
  void DataOps::divide_scalar(EBAMRCellData& a_lhs, const EBAMRCellData& a_rhs){
    for (int lvl = 0; lvl < a_lhs.size(); lvl++){
      DataOps::divide_scalar(*a_lhs[lvl], *a_rhs[lvl]);
    }
  }

  void DataOps::divide_scalar(LevelData<EBCellFAB>& a_lhs, const LevelData<EBCellFAB>& a_rhs){
  ...
  void DataOps::kappa_sum(Real& a_mass, const LevelData<EBCellFAB>& a_lhs){
  ...
  void DataOps::kappa_scale(EBAMRCellData& a_data){
    for (int lvl = 0; lvl < a_data.size(); lvl++){
      DataOps::kappa_scale(*a_data[lvl]);
    }
  }

  void DataOps::kappa_scale(LevelData<EBCellFAB>& a_data){
    EBLevelDataOps::kappaWeight(a_data);
  }

  void DataOps::kappa_scale(MFAMRCellData& a_data){
    for (int lvl = 0; lvl < a_data.size(); lvl++){
      DataOps::kappa_scale(*a_data[lvl]);
    }
  }

  void DataOps::kappa_scale(LevelData<MFCellFAB>& a_data){
  ...
  void DataOps::gen_laplacian(EBAMRCellData& a_lapl, const EBAMRCellData& a_data){
    for (int lvl = 0; lvl < a_data.size(); lvl++){
      DataOps::gen_laplacian(*a_lapl[lvl], *a_data[lvl]);
    }
  }

  void DataOps::gen_laplacian(LevelData<EBCellFAB>& a_lapl, const LevelData<EBCellFAB>& a_data){
    const int ncomp = a_data.nComp();
  ...
  void DataOps::flash_error(EBAMRCellData& a_lapl, const EBAMRCellData& a_data, const Real a_eps){
    for (int lvl = 0; lvl < a_data.size(); lvl++){
      DataOps::flash_error(*a_lapl[lvl], *a_data[lvl], a_eps);
    }
  }

  void DataOps::flash_error(LevelData<EBCellFAB>& a_lapl, const LevelData<EBCellFAB>& a_data, const Real a_eps){
    const int ncomp = a_data.nComp();
  ...
  void DataOps::multiply_scalar(EBAMRCellData& a_lhs, const EBAMRCellData& a_rhs){
    for (int lvl = 0; lvl < a_lhs.size(); lvl++){
      DataOps::multiply_scalar(*a_lhs[lvl], *a_rhs[lvl]);
    }
  }

  void DataOps::multiply_scalar(LevelData<EBCellFAB>& a_lhs, const LevelData<EBCellFAB>& a_rhs){
    CH_assert(a_rhs.nComp() == 1);
  ...
  void DataOps::multiply_scalar(EBAMRIVData& a_lhs, const EBAMRIVData& a_rhs){
    for (int lvl = 0; lvl < a_lhs.size(); lvl++){
      DataOps::multiply_scalar(*a_lhs[lvl], *a_rhs[lvl]);
    }
  }
  
  void DataOps::multiply_scalar(LevelData<BaseIVFAB<Real> >& a_lhs, const LevelData<BaseIVFAB<Real> >& a_rhs){
    CH_assert(a_rhs.nComp() == 1);
  ...
  void DataOps::vector_length(EBAMRCellData& a_lhs, const EBAMRCellData& a_rhs){
    for (int lvl = 0; lvl < a_lhs.size(); lvl++){
      DataOps::vector_length(*a_lhs[lvl], *a_rhs[lvl]);
    }
  }

  void DataOps::vector_length(LevelData<EBCellFAB>& a_lhs, const LevelData<EBCellFAB>& a_rhs){
    CH_assert(a_lhs.nComp() == 1);
    ...
    for (DataIterator dit = a_lhs.dataIterator(); dit.ok(); ++dit){
      ...
      DataOps::vector_length(lhs, rhs, box);
    }
  }

  void DataOps::vector_length(EBCellFAB& a_lhs, const EBCellFAB& a_rhs, const Box& a_box){
    CH_assert(a_lhs.nComp() == 1);
    CH_assert(a_rhs.nComp() == SpaceDim);
  ...
  void DataOps::vector_length2(EBAMRCellData& a_lhs, const EBAMRCellData& a_rhs){
    for (int lvl = 0; lvl < a_lhs.size(); lvl++){
      DataOps::vector_length2(*a_lhs[lvl], *a_rhs[lvl]);
    }
  }

  void DataOps::vector_length2(LevelData<EBCellFAB>& a_lhs, const LevelData<EBCellFAB>& a_rhs){
    CH_assert(a_lhs.nComp() == 1);
    CH_assert(a_rhs.nComp() == SpaceDim);
    ...
    for (DataIterator dit = a_lhs.dataIterator(); dit.ok(); ++dit){
      ...
      DataOps::vector_length2(lhs, rhs, box);
    }
  }

  void DataOps::vector_length2(EBCellFAB& a_lhs, const EBCellFAB& a_rhs, const Box& a_box){
    CH_assert(a_lhs.nComp() == 1);
    CH_assert(a_rhs.nComp() == SpaceDim);
  ...
  void DataOps::compute_min_valid_box(RealVect& a_lo, RealVect& a_hi, const RealVect a_normal, const RealVect a_centroid){
	...
	bool allInside = DataOps::all_corners_inside_eb(corners, a_normal, a_centroid);

	while(allInside){

	  // Shift the corners
	  DataOps::shift_corners(corners, base_shift);
	  shift_vector += base_shift;

	  // Check if shifted corners are inside EB
	  allInside = DataOps::all_corners_inside_eb(corners, a_normal, a_centroid);
  ...
  bool DataOps::all_corners_inside_eb(const Vector<RealVect>& a_corners, const RealVect a_normal, const RealVect a_centroid){
    bool ret = true;
  ...
  void DataOps::shift_corners(Vector<RealVect>& a_corners, const RealVect& a_distance){
    for(int i = 0; i < a_corners.size(); i++){
      a_corners[i] += a_distance;
    }
  }

  void DataOps::filter_smooth(EBAMRCellData& a_lhs, const EBAMRCellData& a_rhs, const int a_stride, const Real a_alpha){
    for (int lvl = 0; lvl < a_lhs.size(); lvl++){
      DataOps::filter_smooth(*a_lhs[lvl], *a_rhs[lvl], a_stride, a_alpha);
    }
  }

  void DataOps::filter_smooth(LevelData<EBCellFAB>& a_lhs, const LevelData<EBCellFAB>& a_rhs, const int a_stride, const Real a_alpha){
    ...
    // Dummy check. 
    for (int dir = 0; dir < SpaceDim; dir++){
      if(ghostVec[dir] < a_stride) MayDay::Abort("DataOps::filter_smooth - stride is too large!");
    }
  ...
  void DataOps::compute_particle_weights(unsigned long long&      a_weight,
