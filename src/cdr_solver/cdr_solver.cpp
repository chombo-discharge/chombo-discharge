...
void cdr_solver::upwind_source(EBAMRCellData& a_dst, const EBAMRCellData& a_src){
  CH_TIME("cdr_solver::upwind");
  if(m_verbosity > 5){
    pout() << m_name + "::upwind" << endl;
  }

  data_ops::copy(a_dst, a_src);
  m_amr->average_down(a_dst, m_phase);
  m_amr->interp_ghost(a_dst, m_phase);
  ...
  if(m_mobile){
    ...
    for (int lvl = 0; lvl <= finest_level; lvl++){
      ...
      for (DataIterator dit = dbl.dataIterator(); dit.ok(); ++dit){
	EBCellFAB& dst         = (*a_dst[lvl])[dit()];
	const EBCellFAB& src   = (*a_src[lvl])[dit()];
	...
	const BaseFab<Real>& vel_reg = velo.getSingleValuedFAB();
...
#if 0
	  for (int dir = 0; dir < SpaceDim; dir++){
	    if(Abs(velo(vof, dir)) > 0.0){
	      Vector<VolIndex> closeVofs;
	      if(velo(vof, dir) > 0.0) closeVofs = ebisbox.getVoFs(vof, dir, Side::Lo, 1);
	      if(velo(vof, dir) < 0.0) closeVofs = ebisbox.getVoFs(vof, dir, Side::Hi, 1);

	      // Take the average over all Vofs on upwind side
	      if(closeVofs.size() > 0){
		for (int i = 0; i < num_vofs; i++){
		  dst(vof, comp) = dst(vof, comp) + (src(vof, comp) - src(closeVofs[i], comp))/num_vofs;
		}
	      }
	    }
	  }
