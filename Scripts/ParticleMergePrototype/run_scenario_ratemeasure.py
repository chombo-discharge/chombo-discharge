...
def run_one_round(comm, rank, size, patches, my_patches, local, thresh, ghost_width, cell_dx, shuffle_rng, id_counter):
    rate_stats = {"n_trivial_shaped": 0, "n_propose_shaped": 0}
    ...
    for pid in patch_order:
        ...
        for d2, p, nbr in edges:
            ...
            if nbr["patch"] == pid:
                ...
                trivial_ok = False
            # RATE-MEASUREMENT INSTRUMENTATION (not part of the validated protocol): tally which
            # "shape" this candidate had BEFORE the dynamic threshold recheck below, so we can
            # compare completion rates between locally-shaped and propose-routed candidates.
            if trivial_ok:
                rate_stats["n_trivial_shaped"] += 1
            else:
                rate_stats["n_propose_shaped"] += 1
    ...
    return final_local, len(trivial_merges), len(judge_merges), rate_stats
...
def main():
    ...
    ap.add_argument("--interior-cluster", action="store_true",
                     help="also seed a same-density cluster at each patch's own center, for a fair "
                          "interior-vs-boundary completion-rate comparison")
    args = ap.parse_args()
    ...
    for pid in my_patches:
        ...
        # RATE-MEASUREMENT ADDITION (not part of the validated scenarios): also drop a cluster of
        # the SAME density at this patch's own geometric center -- guaranteed far from any patch
        # edge -- so there's a genuinely comparable population of crowded INTERIOR cells to weigh
        # against the crowded boundary-cluster cells above. Without this, these scenarios have
        # essentially zero trivial-tier activity (they were built to stress the hard cross-rank
        # path, not to sample "ordinary" interior crowding), so any interior-vs-boundary
        # completion-rate comparison would be measuring noise.
        if args.interior_cluster:
            lo, hi, _rank = box
            center = tuple((lo[d] + hi[d]) / 2 for d in range(len(lo)))
            parts += ml.gen_particles_for_patch(pid, box, gen_rng, n_interior=0,
                                                  n_boundary_cluster=args.n_cluster,
                                                  boundary_point=center, cluster_radius=args.cluster_radius)
        local[pid] = parts
    ...
    for r in range(args.rounds):
        local, n_trivial, n_judge, rate_stats = run_one_round(
        ...
        n_trivial_shaped_g = comm.reduce(rate_stats["n_trivial_shaped"], op=MPI.SUM, root=0)
        n_propose_shaped_g = comm.reduce(rate_stats["n_propose_shaped"], op=MPI.SUM, root=0)
        ...
        if rank == 0:
            round_stats.append({"round": r, "n_particles": n_global, "n_trivial": n_trivial_g,
                                 ...
                                 "n_trivial_shaped": n_trivial_shaped_g, "n_propose_shaped": n_propose_shaped_g})
    ...
    if rank == 0:
        ...
        # RATE-MEASUREMENT SUMMARY (not part of the validated protocol -- ad hoc instrumentation
        # to compare per-round completion rate for locally-shaped vs propose-routed candidates).
        # "shaped" = classified that way at the moment it was popped for processing, BEFORE the
        # dynamic threshold recheck; "completed" = actually resulted in a merge this round.
        n_trivial_shaped_total = sum(rs["n_trivial_shaped"] for rs in round_stats)
        n_propose_shaped_total = sum(rs["n_propose_shaped"] for rs in round_stats)
        n_trivial_completed_total = sum(rs["n_trivial"] for rs in round_stats) * 2  # 2 particles/merge
        n_propose_completed_total = sum(rs["n_judge"] for rs in round_stats)  # 1 proposer wins/merge
        trivial_rate = (n_trivial_completed_total / n_trivial_shaped_total) if n_trivial_shaped_total else float("nan")
        propose_rate = (n_propose_completed_total / n_propose_shaped_total) if n_propose_shaped_total else float("nan")
        print(f"[RATE] scenario={args.scenario} seed={args.seed} shuffle={args.shuffle_seed} "
              f"trivial_shaped={n_trivial_shaped_total} trivial_completed_particles={n_trivial_completed_total} "
              f"trivial_rate={trivial_rate:.3f} propose_shaped={n_propose_shaped_total} "
              f"propose_completed_proposers={n_propose_completed_total} propose_rate={propose_rate:.3f}")
