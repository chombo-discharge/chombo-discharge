...
Milestone 1 is #730 and this claim still holds of it. The programme has since needed Chombo changes
for Milestone 3 — Chombo-3.3#31, the index-space side of #731 — which does not weaken the claim
here, but a cold reader should not take "no changes to Chombo" as describing the whole of the work.
...
- **Milestone 1** — this file, and #730. Replace the moment machinery, behind the switch. No Chombo
  changes, no refinement, no new EBIS.
- **Milestones 2 and 3** — `PLAN_MILESTONES.md`. Refining parent cells on top of Milestone 1
  (#732), then AMR-aware geometry generation where planar regions stay coarse and curved regions
  refine (#731, built, with Chombo-3.3#31). They were written in that order and built in the other:
  3 turned out not to need 2, and what 2 still owes 3 is in `HANDOFF.md` §7.
