...
RodNeedlePlane::defineRodWNeedle()
{
  ...
  electrodeParts.push_back((BaseIF*)new NeedleIF(length, needleRadius, tipRadius, angle, false));
...
RodNeedlePlane::defineNeedle()
{
  ...
  RefCountedPtr<BaseIF> bif = RefCountedPtr<BaseIF>(new NeedleIF(length, radius, tipRadius, angle, false));
