...
``Random`` is a static class for generating pseudo-random numbers, and exists so that all random number operations can be aggregated into a single class.
Internally, ``Random`` uses a Mersenne-Twister random number generator supplied by ``std::random``. 
...
``Driver`` (see :ref:`Chap:Driver`) will seed the random number generator, and the user can override the seed by setting ``Random.seed = <number>`` in the input script.
