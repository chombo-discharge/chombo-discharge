...
* ``FieldSolverGMG.solver``.
  Selects the top-level solver: ``gmg``, ``gmres``, or ``bicgstab``; a space-separated list (e.g. ``gmg gmres``) is a fallback chain tried in order. The Krylov options use the V-cycle as a preconditioner and are configured with the ``krylov_*`` keys; see :ref:`Chap:KrylovMultigrid`. This key and the ``krylov_*`` keys are mandatory.
